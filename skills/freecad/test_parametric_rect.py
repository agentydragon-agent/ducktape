...
_OUT_DIR = "freecad-parametric-rect"


def _save_output(name: str, content: str) -> None:
    out_dir = undeclared_outputs_dir() / _OUT_DIR
...
def test_parametric_rect(tmp_path: Path) -> None:
    ...
    with DockerContainer(
        ...
        _save_output("container-stdout.log", stdout.decode(errors="replace"))
        _save_output("container-stderr.log", stderr.decode(errors="replace"))
