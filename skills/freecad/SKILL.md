...
| `getVisibleEdges` coords vs scale    | `getVisibleEdges()` returns edge coordinates in **model space** (mm), NOT scaled by `view.Scale`. Compare edge lengths against raw sketch dimensions (no `* scale`). But `dim.X`/`dim.Y` (label placement) ARE in view-local coords = model coords × scale                                               |
