...
This skill does exactly one thing: rename symbols whose YAML `name:` still
equals their `selector.binding.name` (the minified input-bundle name). It
does **not** move modules, redraw boundaries, edit taxonomy, or run cycle
gates.
...
for root, dirs, files in os.walk('modules'):
  for fn in files:
    if not fn.endswith('.yaml') or fn.endswith('.yaml.deferred'):
      continue
...
in the emitted JS that has no YAML binding yet (still living in the
residual / deferred backlog, or not attributed to any module). Don't go
...
- **Editing anything besides `name:`**: this skill does not move members,
  change selectors, add purity hints, or create new modules.
