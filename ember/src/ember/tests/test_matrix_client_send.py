...
@pytest.mark.asyncio
async def test_matrix_client_send_text_message(
    ...
    async def fake_sync_loop(
        self: MatrixClient,
    ) -> None:  # pragma: no cover - exercised via task
        await asyncio.sleep(0)

    monkeypatch.setattr(
        MatrixClient, "_create_client", fake_create_client, raising=False
    )
    monkeypatch.setattr(
        MatrixClient,
        "_initialise_control_rooms",
        fake_initialise_control_rooms,
        raising=False,
    )
