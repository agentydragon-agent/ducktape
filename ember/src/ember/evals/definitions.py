...
class Scenario(ABC):
    ...
    async def wait_for_matrix_response(
        ...
        result = await self._require_executor().wait_for_matrix_response(
            sender=sender,
    ...
    async def expect_matrix_reply(
        self, equals: str, *, timeout_seconds: int = 60
    ) -> ExpectMatrixReplyResult:
        result = await self._require_executor().expect_matrix_reply(
            equals, timeout_seconds=timeout_seconds
        )
        self.record(result)
        return result
    ...
    def fail(self, message: str) -> None:
        ...
        from .executor import ScenarioExecutionError
