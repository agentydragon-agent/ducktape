...
class ScenarioExecutor:
    ...
    async def wait_for_matrix_response(
        ...
        *,
        ...
        self._last_matrix_message = await self._matrix.wait_for_message(
            sender=sender,
            timeout_seconds=timeout_seconds,
        )
        return WaitForMatrixResponseResult(
            sender=self._last_matrix_message.sender,
            body=self._last_matrix_message.body,
        )

    async def expect_matrix_reply(
    ...
    def validate_last_matrix_regex(
        ...
        *,
        ...
        return ValidateRegexResult(
            pattern=pattern,
            flags=flags,
            timezone_tolerance_days=timezone_tolerance_days,
        )
    ...
    async def run_in_container_binary(
        self,
        container: str | None,
        command: Sequence[str],
    ) -> bytes:
    ...
    async def probe_http(
        ...
        *,
        ...
        if expect_body_includes and expect_body_includes not in body:
            raise ScenarioExecutionError(
                f"Body missing expected content: {expect_body_includes!r}"
            )
    ...
    async def snapshot_workspace(
        ...
        tar_bytes = await self.run_in_container_binary(
            None,
            [
                "tar",
                "czf",
    ...
    async def kill_process(self, *, container: str, pattern: str) -> KillProcessResult:
        ...
        if result.returncode not in (0, 1):
            raise ScenarioExecutionError(
                f"pkill failed: {result.stderr.strip() if result.stderr else result.stdout}"
            )
        ...
        for index, scenario_cls in enumerate(suite.scenarios, start=1):
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"[ember-eval][{self._request.run_id}] teardown for {scenario.id} raised: {exc}",
                        flush=True,
                    )
                scenario_results.append(result)
                print(
                    f"[ember-eval][{self._request.run_id}] ⇠ Scenario {index} completed with status {status.value}"
                )
    ...
    def _verify_timestamps(
        ...
        if len(lines) < minimum_entries:
            raise ScenarioExecutionError(
                f"Expected at least {minimum_entries} timestamps, saw {len(lines)}"
            )
        parsed = []
        for line in lines:
            try:
