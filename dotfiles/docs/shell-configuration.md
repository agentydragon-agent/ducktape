...
| File | Purpose | When Loaded | What Goes Here |
|------|---------|-------------|----------------|
| `~/.profile` | POSIX environment setup | Login shells (bash, sh) | PATH, environment variables, sources ~/.secret_env |
| `~/.bashrc` | Bash interactive config | Interactive bash shells | Sources ~/.shellrc, bash-specific settings |
| `~/.zshenv` | Zsh environment (minimal) | ALL zsh invocations | Only zsh-specific settings (skip_global_compinit) |
| `~/.zprofile` | Zsh login config | Zsh login shells | Currently empty |
| `~/.zshrc` | Zsh interactive config | Interactive zsh shells | Sources ~/.shellrc, oh-my-zsh, zsh-specific |
| `~/.shellrc` | Interactive settings | Interactive shells | Sources profile, aliases, functions, completions |
| `~/.secret_env` | Secret environment vars | When sourced by ~/.profile | API keys, tokens (not in git) |
