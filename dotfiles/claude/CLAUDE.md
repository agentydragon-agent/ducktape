...
Outcome: Discovered tokens expire in 1hr not 24hr" > experiments/2024-01-15-auth-debugging/README.md
  ...
  <sec id="/task-tool" title="Task Tool Critical Behavior and Best Practices">
    ...
    <sec title="⚠️ TASKS MAY BE ABORTED WITHOUT WARNING" priority="CRITICAL">
      <ul>
    ...
    <ex-parallelization>
      ...
      <desc>All tasks run in parallel, you regain control only after all complete (or are aborted)</desc>
      
      <ex id="with-file-copies">
        <title>Safe parallel editing with file copies</title>
        <desc>First set up isolated work areas:</desc>
        <setup>
  ...
  <task-management id="/todowrite-everything">
    ...
    <desc>TodoWrite is Claude's external brain - use it CONSTANTLY</desc>
    <critical>
