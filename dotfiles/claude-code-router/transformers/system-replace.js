 ...
 */
// TODO(mpokorny): Support agent-mode variant ("You are an agent for Claude Code, Anthropic's official CLI for Claude.") by adding a second static template and runtime selection that preserves byte-identity for agent-mode samples.
...
function identityRecomposer(s) {
  ...
  function regexEscape(x) { return x.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); }
  function extractPromptBlobs(t) {
    const out = { env_git_blobs: [] };
    if (typeof t !== 'string') return out;
    const envBlockRe = new RegExp(regexEscape(ENV_INTRO) + '\\n<env>[\\s\\S]*?<\\/env>\\s*', 'g');
    let m;
    while ((m = envBlockRe.exec(t))) out.env_git_blobs.push(m[0]);
    const iTools = t.indexOf(TOOLS_HEADER);
    if (iTools !== -1) {
      const after = iTools + TOOLS_HEADER.length;
      const nextEnv = t.indexOf(ENV_INTRO, after);
      const nextModel = t.indexOf(MODEL_PREFIX, after);
      let end = t.length;
      if (nextEnv !== -1) end = Math.min(end, nextEnv);
      if (nextModel !== -1) end = Math.min(end, nextModel);
      out.tools_blob = t.slice(after, end);
    }
    const mm = t.match(new RegExp('^' + regexEscape(MODEL_PREFIX) + '[^\\n]*\\n?', 'm'));
    if (mm) out.model_line = mm[0];
    const iM = t.indexOf(MCP_HEADER);
    if (iM !== -1) {
      const nl = t.indexOf('\n', iM);
      out.mcp_section = nl === -1 ? '' : t.slice(nl + 1);
    }
    return out;
  }
  const TEMPLATE = `You are an interactive CLI tool that helps users with software engineering tasks. Use the instructions below and the tools available to you to assist the user.
...
{{MCP_SECTION}}`;
  ...
  // Only recompose on the main tools block section.
  // Anthropic splits the system across sections; MCP-only sections must pass through unchanged
  // to avoid duplicating the header in later sections.
  if (!s.includes('You can use the following tools')) return s;
...
You can use the following tools without requiring user approval:${b.tools_blob}
${Array.isArray(b.env_git_blobs) ? b.env_git_blobs.join('') : ''}
${b.model_line ?? ''}
...
module.exports = Object.assign(SystemMessageTransformer, { identityRecomposer, renderFromStaticTemplate, extractPromptBlobs });
