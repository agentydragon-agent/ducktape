...
from tools.claude_hooks import statusline_models as sl
from tools.claude_hooks.statusline import _format_quota
from tools.claude_hooks.usage_api import (
    CACHE_TTL_SECONDS,
    UsageBucket,
    UsageResponse,
    _CachedUsage,
...
def test_read_access_token(tmp_path: Path):
    ...
    with patch("tools.claude_hooks.usage_api.CREDENTIALS_PATH", creds_file):
        assert _read_access_token() == "test-token-123"


def test_read_access_token_missing_file(tmp_path: Path):
    with patch("tools.claude_hooks.usage_api.CREDENTIALS_PATH", tmp_path / "nonexistent"):
...
def test_get_cached_usage_fresh_cache(tmp_path: Path):
    ...
    with patch("tools.claude_hooks.usage_api.CACHE_PATH", cache_file):
...
def test_get_cached_usage_stale_cache_no_token(tmp_path: Path):
    ...
    with (
        patch("tools.claude_hooks.usage_api.CACHE_PATH", cache_file),
        patch("tools.claude_hooks.usage_api.CREDENTIALS_PATH", creds_file),
    ):
        result = get_cached_usage()
...
def test_get_cached_usage_no_cache_no_token(tmp_path: Path):
    with (
        patch("tools.claude_hooks.usage_api.CACHE_PATH", tmp_path / "nonexistent"),
        patch("tools.claude_hooks.usage_api.CREDENTIALS_PATH", tmp_path / "also_nonexistent"),
    ):
        result = get_cached_usage()
