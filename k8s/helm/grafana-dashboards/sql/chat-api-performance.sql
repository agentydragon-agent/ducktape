...
success_samples AS (
  SELECT DISTINCT ON (model)
    ...
    LEFT(
          (
            ...
            FROM jsonb_array_elements(chat_response_content::jsonb->'choices') AS item
  ...
  WHERE $__timeFilter(start_time)
    ...
    AND family LIKE '$family'
    AND success = true
    AND chat_response_content IS NOT NULL
    AND chat_response_content != ''
    AND chat_response_content::jsonb ? 'choices'
