...
MODEL_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/npu-llm/models"
mkdir -p "$MODEL_DIR"
...
case "$1" in
  ...
  export)
    ...
    ensure_venv
    out="$(model_dir_for "$2")"
    exec "$VENV_DIR/bin/python" "$SCRIPT_DIR/local-llm-npu-export.py" "$2" "$out"
    ;;
  chat)
    [ $# -lt 2 ] && usage
    ensure_venv
    ...
    exec "$VENV_DIR/bin/python" "$SCRIPT_DIR/local-llm-npu-chat.py" "$out"
    ;;
  bench)
    [ $# -lt 2 ] && usage
    ensure_venv
    out="$(model_dir_for "$2")"
    require_exported "$out" "$2"
    exec "$VENV_DIR/bin/python" "$SCRIPT_DIR/local-llm-npu-bench.py" "$out"
    ;;
  list)
  ...
  server)
    ...
    exec "$VENV_DIR/bin/python" "$SCRIPT_DIR/local-llm-npu-server.py" "$out"
    ;;
  *)
    usage
    ;;
