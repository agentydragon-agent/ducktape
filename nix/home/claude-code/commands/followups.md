...
git diff path/to/file.py | grep "expected_change_marker"
# or
grep "distinctive_line_added" path/to/file.py
...
patterns = [
    r"should (\w+)",
    r"TODO[:\s]+(.+)",
    r"(?:later|next|future).*?(\w+)",
    r"consider (\w+ing)",
    r"might want to (\w+)",
]
```

### Code Propagation Search

```bash
# New pattern created
rg "OldPattern" --type py  # sites to update

# Duplication removed
rg "removed_duplicate_code" --type py  # check for more

# Bug fixed
rg "similar_buggy_pattern" --type py  # related bugs
```
