...
let
  ...
  # Combine all commands from both SSOTs (already in uniform { type, cmd } format)
  allCommands = inspection.exports.noSudo ++ inspection.exports.sudo ++ allowed.noSudo;

  # Transform simple { type, cmd } → Bash() permission
  # Input: { type = "prefix"|"exact"; cmd = "full command string"; }
  # Output: "Bash(cmd:*)" or "Bash(cmd)" depending on type
  toBashPerm =
    entry:
    let
