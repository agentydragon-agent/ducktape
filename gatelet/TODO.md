...
- For Home Assistant:
  ...
  - Show entities grouped by their area (from HA; again, for *human only*, linked to HA instance). Represent entities also by their display name so model better
    understands.
  - Show values from all continuous sensors on dashboard in *one table* to save repeating datetime values, column headers, etc.
...
- Expose ActivityWatch data to LLM. Architecture:
  - Probably: ActivityWatch API server pointer in configuration. Server will aggregate data from *multiple hosts*.
