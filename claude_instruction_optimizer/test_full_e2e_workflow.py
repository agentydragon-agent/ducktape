...
from database import (
    init_database, get_db_session, OptimizationRun, SystemPrompt, 
    SeedTask, GradingCriteria, Rollout, RolloutMessage, RolloutFile,
    GraderRun, GraderFacetResult, PatternAnalysis, PatternAnalysisRollout
)
...
def sample_seeds_yaml(tmp_path):
    """Create a sample seeds.yaml file with realistic tasks."""
    seeds_data = [
        {
            'id': 'test_rest_api',
...
"""
        ...
        with get_db_session() as session:
            ...
            for i, task in enumerate(tasks[:2]):  # Test with 2 tasks
                ...
                output_files = [("api_client.py", "# API client code"), ("main.py", "# Main module")] if "rest" in task.task_id else [("config_loader.py", "# Config loader code")]
...
- Reasonable error handling strategies""",
        ...
        with get_db_session() as session:
            ...
            assert len(rollouts) == 4  # 2 tasks × 2 iterations
            ...
            assert len(facet_results) == 12  # 4 rollouts × 3 criteria each
            ...
            type_criteria = session.query(GradingCriteria).filter(
        ...
        print("\n✅ Full end-to-end workflow test completed successfully!")
        ...
        print(f"   - All data relationships and queries working correctly")
