...
from database import (
    ...
    GraderRun, GraderFacetResult, PatternAnalysis
)
...
def sample_seeds_yaml(tmp_path):
    """Create a sample seeds.yaml file."""
    seeds_data = [
        {
            'id': 'test_task_001',
...
class TestYamlDatabaseSync:
    ...
    def test_content_hash_change_detection(self, temp_db, sample_seeds_yaml):
        ...
        with get_db_session() as session:
            ...
            # Modify YAML file
            with open(sample_seeds_yaml, 'r') as f:
                data = yaml.safe_load(f)
            data[0]['prompt'] = 'MODIFIED: Create a different REST API client.'
            with open(sample_seeds_yaml, 'w') as f:
                yaml.dump(data, f)
...
class TestDatabaseModels:
    ...
    def test_rollout_file_integrity_checking(self, temp_db, tmp_path):
        ...
        with get_db_session() as session:
            ...
            # Test that modified file fails integrity check
