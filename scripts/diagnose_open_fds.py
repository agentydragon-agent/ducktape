...
def format_process_summary(info: ProcessInfo) -> str:
    ...
    return (
        f"{info.fd_count:6} FDs | pid={info.pid} | user={info.username} | "
        f"{info.comm}{percent_of_limit}"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Diagnose open file descriptor usage system-wide."
    )
    parser.add_argument(
        "--top",
        type=int,
        default=10,
        help="Number of top processes to display in detailed output (default: 10).",
    )
    parser.add_argument(
        "--pid",
        type=int,
        action="append",
        help="Inspect these specific PIDs regardless of ranking (can be repeated).",
    )
    parser.add_argument(
        "--min-fds",
        type=int,
        default=0,
        help="Only display processes with at least this many FDs in the summary.",
    )
...
def main() -> None:
    ...
    if not check_fd_dir_access():
        print(
            "WARNING: Limited visibility into /proc/<pid>/fd; some descriptors may be missing.",
            file=sys.stderr,
        )
    ...
    for info in infos[: args.top]:
        ...
        if info.fd_examples:
            example_parts = [
                f"{category} -> {example}"
                for category, example in list(info.fd_examples.items())[:3]
            ]
    ...
    if args.pid:
        requested = {pid: None for pid in args.pid}
        for info in infos:
