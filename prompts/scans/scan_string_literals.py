...
def build_histogram(
    ...
    sorted_histogram = dict(
        sorted(histogram.items(), key=lambda x: len(x[1]), reverse=True)
    )
    ...
    for value, occurrences in sorted_histogram.items():
        result[value] = {
            ...
            "locations": [
                {k: v for k, v in occ.items() if k != key} for occ in occurrences
            ],
        }
...
def find_overlaps(
    literal_histogram: dict[str, dict], symbol_histogram: dict[str, dict]
) -> dict[str, dict]:
