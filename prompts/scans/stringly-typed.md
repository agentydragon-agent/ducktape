...
**Critical**: If you have a structured error field, USE IT. Don't just shove composed strings into `message`:

```python
# BAD: Wasted structured error - just composed strings
error=ErrorPayload(
    message=f"Upstream status {resp.status_code}",  # NO!
)

# GOOD: Use detail field for structured data
error=ErrorPayload(
...
You can now query: "show all errors where status_code >= 500" - impossible with string messages!
