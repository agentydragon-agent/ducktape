...
def scan_directory(root: Path) -> dict[str, Any]:
    ...
    return {
        "summary": {
            ...
            "files_with_comments": len(comments_by_file),
        },
...
def main() -> None:
    ...
    print(
        f"Total comments/docstrings: {results['summary']['total_comments']}",
        file=sys.stderr,
    )
    print(
        f"Files with comments: {results['summary']['files_with_comments']}",
        file=sys.stderr,
    )
