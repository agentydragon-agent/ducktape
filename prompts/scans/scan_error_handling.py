...
class ErrorHandlingAnalyzer(ast.NodeVisitor):
    ...
    def visit_Try(self, node: ast.Try) -> None:
        ...
        meaningful_stmts = [
            s
            for s in node.body
            if not (isinstance(s, ast.Pass) or isinstance(s, ast.Expr))
        ]
        if len(meaningful_stmts) == 1:
            self.single_line_try.append(
                {
                    "line": node.lineno,
                    "col": node.col_offset,
                }
...
def main() -> None:
    ...
    print(
        f"Non-raising except: {results['summary']['non_raising_except']}",
        file=sys.stderr,
    )
    print(
        f"Single-line try: {results['summary']['single_line_try']}", file=sys.stderr
    )
