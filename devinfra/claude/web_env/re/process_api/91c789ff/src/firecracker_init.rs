...
//!
//! Removed in 91c789ff:
//!   "[INIT] Snapstart template mode: signaling ready..."  (snapstart removed)
//!
...
/// 3. Write /etc/hostname, /etc/hosts, /etc/resolv.conf (nameserver 8.8.8.8)
///    (Binary: 91c789ff — DNS/network config set up at init time)
/// 4. Read /mount_config.json (fresh boot)
///    (Snapstart template mode removed in 91c789ff)
/// 5. Mount root filesystem from /dev/vda, pivot_root or MS_MOVE+chroot
/// 6. Mount model tools from /dev/vdb if configured
/// 7. Create /dev/fuse, set up FUSE mounts via rclone (fuse_spawn)
/// 8. Mount rclone_tools
/// 9. Mount readonly block devices
/// 10. Write config files (etc_hosts, resolv_conf, ca_cert_pem)
/// 11. Load environment variables from /container.env
/// 12. Scrub auth tokens from configs
/// 13. Drop CAP_SYS_RESOURCE
/// 14. Write drop_caches
pub fn run_firecracker_init() {
    ...
    // Step 3: Write DNS/network config files at init time.
    ...
    // Step 4: Read /mount_config.json (fresh boot only).
    // Binary: 91c789ff — snapstart template mode removed.
    ...
    // Steps 5-14: Apply mount configuration
...
pub fn apply_mount_config(config: &MountRootConfig) -> Result<String, String> {
    ...
    // Set up FUSE mounts
    // Binary: 91c789ff — status strings changed from "fuse_mounts" to "fuse_spawn"
