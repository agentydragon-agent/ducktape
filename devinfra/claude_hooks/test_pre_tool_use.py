...
from devinfra.claude_hooks.pre_tool_use import PreToolUseInput, evaluate
...
class TestEvaluate:
    def test_allowed_bash_command_returns_allow(self) -> None:
        hook_input = PreToolUseInput(**COMMON_FIELDS, tool_name="Bash", tool_input={"command": "echo hello world"})
        result = evaluate(hook_input)
        assert result is not None
        assert result.hook_specific_output.permission_decision == "allow"
    ...
    def test_unknown_bash_command_returns_none(self) -> None:
        hook_input = PreToolUseInput(**COMMON_FIELDS, tool_name="Bash", tool_input={"command": "rm -rf /"})
        assert evaluate(hook_input) is None

    def test_non_bash_tool_returns_none(self) -> None:
        hook_input = PreToolUseInput(**COMMON_FIELDS, tool_name="Read", tool_input={"file_path": "/etc/passwd"})
        assert evaluate(hook_input) is None

    def test_bash_without_command_key_returns_none(self) -> None:
        hook_input = PreToolUseInput(**COMMON_FIELDS, tool_name="Bash", tool_input={})
        assert evaluate(hook_input) is None
