...
## Owner-graph side-effect classification: emit reasoning

`owner_graph.json` exposes `has_side_effect: true|false` per owner
node but no rationale. When a spec author or peel-loop driver sees
an owner unexpectedly classified side-effecting and has to figure
out *why*, the only path today is: read `purity.rs`, hypothesize,
write a synthetic e2e fixture, differentially test, repeat. That
diagnostic loop is expensive and has to be redone every time a new
construct surfaces.

Replace the bare boolean with a structured rationale. Concretely:

- Per owner with `has_side_effect: true`, attach a `side_effect_reason`
  field (or rename to something with structured payload) describing
  *which sub-expression* triggered the verdict and *which classifier
  rule* fired. E.g. `{ kind: "call_callee_unknown", callee: "foo",
  span: [1234, 1239] }`, or `{ kind: "new_arg_not_pure_array_literal",
  callee: "Map", span: ... }`, or `{ kind: "impure_assign", span: ... }`.
- Plumb the reason through `classify_expr_purity` — add a sibling
  `classify_expr_purity_with_reason` that returns
  `(Purity, Option<Reason>)`, used by the owner-graph builder; the
  existing call sites that only care about the verdict keep using the
  thin wrapper. Or thread an out-param.
- For `new`-expression rules in particular, distinguish "callee not
  whitelisted" vs. "argument shape rejected" (and *which* element
  of the array failed) — that's the diagnostic that today requires
  reading the implementation.
- For `Stmt::Decl(Decl::Var(_))` with multiple declarators, attach
  reasons per declarator, not just per statement — so the consumer
  knows whether `r4`'s side-effect is from `r4`'s init or inherited
  from a sibling declarator (`n4`'s init) under sequence-point
  semantics.

This shows up first as a peel-loop debug aid, but it's also the
right format for a future `--explain <binding>` CLI mode.
