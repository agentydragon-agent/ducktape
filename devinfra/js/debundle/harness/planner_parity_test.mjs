...
function normalize(snapshot) {
  ...
  const extractionGroups = snapshot.extractionGroups.map((g) => [...g].sort()).sort((a, b) => a[0].localeCompare(b[0]));
  return {
    schemaVersion: 1,
    modules,
    selectedModules,
    ...
    debug: {
      selected: (snapshot.debug?.selected ?? [])
        .map((s) => ({
          ...
          semanticBlockingReasons: [...(s.semanticBlockingReasons ?? s.semantic_blocking_reasons ?? [])].sort(),
          stageRuns: normalizeStageRuns(s.stageRuns ?? s.stage_runs ?? []),
        }))
        .sort((a, b) => a.memberNames.join("\n").localeCompare(b.memberNames.join("\n"))),
      orderedInitState: normalizeOrderedInitState(
        snapshot.debug?.orderedInitState ?? snapshot.debug?.ordered_init_state ?? {}
      ),
    },
  };
}
