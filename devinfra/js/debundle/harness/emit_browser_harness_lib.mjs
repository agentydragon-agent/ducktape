...
import { loadVendorResolutionManifest } from "../common/vendor_runtime_lib.mjs";
...
export function parseHarnessArgs(argv) {
  const options = {
    assetSummaryPath: undefined,
    force: false,
    help: false,
    outDir: undefined,
    scriptSource: "snapshot",
    snapshotRoot: undefined,
    vendorManifestPath: undefined,
  };
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    switch (arg) {
      case "--asset-summary":
        options.assetSummaryPath = resolveWorkspacePath(requireValue(argv, ++index, arg));
        break;
      case "--force":
        options.force = true;
        break;
      ...
      case "--vendor-manifest":
        options.vendorManifestPath = resolveWorkspacePath(requireValue(argv, ++index, arg));
        break;
      case "--snapshot-root":
        options.snapshotRoot = resolveWorkspacePath(requireValue(argv, ++index, arg));
        break;
      case "--script-source":
        options.scriptSource = requireValue(argv, ++index, arg);
        break;
      default:
  ...
  if (!options.snapshotRoot) {
    throw new Error("--snapshot-root is required");
  }
  if (!options.assetSummaryPath) {
    throw new Error("--asset-summary is required");
  }
  if (options.scriptSource !== "snapshot" && options.scriptSource !== "split") {
    throw new Error("--script-source must be either snapshot or split");
  }
  if (!options.outDir) {
