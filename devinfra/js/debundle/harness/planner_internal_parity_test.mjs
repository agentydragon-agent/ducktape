...
import {
  getOrderedInitPlannerStateForTesting,
  packSelectedModuleGroups,
  planSelectedModuleGroupExtractions,
} from "../extract/decl_graph.mjs";
...
test("planner internals parity: rust extraction groups exactly match js planner batch groups", async () => {
  ...
  const rustFrontierSummary = (rustPlanner.debug?.frontierTraces ?? rustPlanner.debug?.frontier_traces ?? []).map(
    (trace) => ({
    ...
    if (firstGateDelta) {
      assert.fail(
        [
          ...
          `rust trace@delta: ${JSON.stringify(firstGateDelta.rust?.trace ?? null, null, 2)}`,
          `js trace@delta: ${JSON.stringify(firstGateDelta.js?.trace ?? null, null, 2)}`,
          `rust owner analyses for seed: ${JSON.stringify(
            ownerAnalysesForSeed(
              rustPlanner.debug?.ownerAnalyses ?? rustPlanner.debug?.owner_analyses ?? [],
              firstGateDelta.rust?.seedOwnerIds ?? firstGateDelta.js?.seedOwnerIds ?? []
            ),
            null,
            2
          )}`,
...
function ownerAnalysesForSeed(ownerAnalyses, seed) {
  const seedOwnerIds = Array.isArray(seed) ? seed : seed.split("|").filter(Boolean);
...
function firstFrontierGateDelta(rustFrontier, jsFrontier, projector) {
  const rustRows = frontierGateRows(rustFrontier, projector);
  const jsRows = frontierGateRows(jsFrontier, projector);
  const max = Math.max(rustRows.length, jsRows.length);
  for (let index = 0; index < max; index++) {
    const rust = rustRows[index] ?? null;
    const js = jsRows[index] ?? null;
    if (JSON.stringify(comparableFrontierGateRow(rust)) !== JSON.stringify(comparableFrontierGateRow(js))) {
      return {
        index,
        rust,
        js,
...
function comparableFrontierGateRow(row) {
  if (row === null) return null;
  return {
    seedOwnerIds: row.seedOwnerIds,
    seedMemberNames: row.seedMemberNames,
    seedComponentDepOwnerIds: row.seedComponentDepOwnerIds,
    requiredComponentIds: row.requiredComponentIds,
    values: row.values,
  };
}

function frontierGateRows(frontier, projector) {
  return frontier
    .map((trace) => ({
      seedOwnerIds: trace.seedOwnerIds,
      seedMemberNames: trace.seedMemberNames,
      seedComponentDepOwnerIds: trace.seedComponentDepOwnerIds,
      requiredComponentIds: trace.requiredComponentIds,
      values: [...projector(trace)].sort(),
      trace,
    }))
    .sort((left, right) =>
      JSON.stringify({
        seedOwnerIds: left.seedOwnerIds,
        seedMemberNames: left.seedMemberNames,
        seedComponentDepOwnerIds: left.seedComponentDepOwnerIds,
        requiredComponentIds: left.requiredComponentIds,
        values: left.values,
      }).localeCompare(
        JSON.stringify({
          seedOwnerIds: right.seedOwnerIds,
          seedMemberNames: right.seedMemberNames,
          seedComponentDepOwnerIds: right.seedComponentDepOwnerIds,
          requiredComponentIds: right.requiredComponentIds,
          values: right.values,
        })
      )
    );
...
function normalizeSelectedDebug(records) {
  return records
    .map((record) => ({
      ...
      semanticBlockingReasons: [...(record.semanticBlockingReasons ?? record.semantic_blocking_reasons ?? [])].sort(),
