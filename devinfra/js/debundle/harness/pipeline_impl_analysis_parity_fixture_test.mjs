...
test("rust analysis snapshot matches js boundary semantics on synthetic non-mock fixture", () => {
  ...
  const run = runRustFixture(tmp, inputRoot, outRoot);
  ...
  const js = {
    ...
    modules: [buildModule(aPath, aCode, 0), buildModule(bPath, bCode, 1)].sort((x, y) =>
      x.moduleId.localeCompare(y.moduleId)
    ),
  };
...
function runRustFixture(tmp, inputRoot, outRoot) {
  const jsListPath = join(tmp, "js-files.txt");
  const jsFiles = readFileSync(jsListPath, "utf8")
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
  const entryScript = jsFiles.find((path) => path.includes("index") || path.includes("root")) ?? jsFiles[0];
  writeFileSync(
    join(inputRoot, "index.html"),
    `<!doctype html>\n<html>\n  <head>\n    <meta charset="utf-8" />\n    <script type="module" src="/${entryScript}"></script>\n  </head>\n  <body></body>\n</html>\n`
  );
  writeFileSync(
    join(tmp, "asset-summary.json"),
    `${JSON.stringify(
      {
        uiVersion: "fixture",
        entryPoints: {
    ...
    `${JSON.stringify(
      {
        ...
        pipeline: [
          ...
          },
          {
            ...
            args: {
              assetSummaryPath: join(tmp, "asset-summary.json"),
              force: true,
              outDir: outRoot,
              scriptSource: "split",
  ...
  return spawnSync(resolveRustBin(), ["--spec", join(tmp, "transform-spec.json")], {
    encoding: "utf8",
  });
}
