...
at the same root. The proposed shape uses two top-level categories:
...
## Planned JSON Outputs And Schemas

`app/package.json` is the only planned runtime JSON generated by debundle:
...
This should be a backward-incompatible cutover. Do not emit old-path aliases,
dual manifests, compatibility readers, or deprecation shims. The cutover should
land as one coherent Ducktape change, followed by a Gaffer repin that updates
the consuming Tana targets to the new contract.

### 1. Freeze The Contract

- Treat this file as the provisional source of truth while implementing.
- Before editing code, reconcile the final names and fields here with any last
  reviewer notes.
- Add focused tests that assert the new output tree shape, especially absence
...
- Add a small output-layout helper used by `emit_harness`, `write_tree`,
  `spec_tree`, `materialize_logical_modules`, vendor swap output, directory
  reports, and identifier rename queue output.
- The helper should expose semantic paths such as runtime root, reports root,
  tree reports root, chunk report directory, vendor swap report, and rename
  queue report.
- Avoid scattering string literals for `app/`, `reports/`, `reports/tree/`,
  `chunk.json`, `modules.json`, and `owner_graph.json`.

### 3. Move Runtime Output

- Make browser-loadable output land under `app/`.
- Update generated HTML/bootstrap paths and module specifier rewriting so the
  app still loads from `app/` as its runtime root.
- Keep generated vendor wrapper JS under `app/vendors/generated/**`.
- Keep `app/package.json` as runtime package metadata.
...
- Update Tana-specific spec targets, live proxy, load tests, generated snapshot
  copy logic, and `DEBUNDLE_AGENT_CONTEXT.md` to the new paths.
