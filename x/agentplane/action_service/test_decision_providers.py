    ...
    return ActionCatalog(
        groups={
            "test_fixture": ActionGroup(
                ...
                executor=ExecutorBinding(kind="mcp", description="Test server"),
...
@pytest.mark.parametrize("kind", [None, "hostexec"])
def test_fixture_provider_rejects_missing_or_non_mcp_group(fixture_catalog: ActionCatalog, kind: str | None) -> None:
    if kind is None:
        fixture_catalog.groups.clear()
    else:
        fixture_catalog.groups["test_fixture"].executor.kind = kind
...
class FixtureExecutor:
    @property
    def capabilities(self) -> frozenset[str]:
        return frozenset({"test_fixture.fixture_info"})
...
async def test_fixture_provider_uses_existing_deny_dominant_decision_path(
    engine: AsyncEngine, fixture_provider: FixtureDecisionProvider, deny: bool
) -> None:
    store = ActionStore(make_sessionmaker(engine))
    service = ActionService(
        store,
...
async def test_outside_fixture_scope_keeps_human_fallback_despite_spoofed_provenance(
    engine: AsyncEngine, fixture_provider: FixtureDecisionProvider
) -> None:
    service = ActionService(ActionStore(make_sessionmaker(engine)), FixtureExecutor(), providers=[fixture_provider])
    try:
        result = await service.submit(
            ActionRequestInput(
