...
Status: **native drivers, scripted harness tests, and the first shared bidirectional gRPC runner
are in place; the runner is now being hardened against provider edge cases and review feedback**.
...
The implemented seam is one bidirectional gRPC stream plus a Claude adapter and a Codex adapter.
It exposes only operations and observations that the scripted harness tests can prove with the real
binaries. The client-facing surface is:

```text
Connect(Start)
  Input(input_id, text, mode)
  Interrupt(command_id, reason, cancel_queued)
  Close(reason)

  Event(sequence, observation, source_event_sequences)
```

The seam reports provider capabilities and provider-specific terminal/error outcomes. It must not
silently turn a second input into steering, emulate retry, redispatch an uncertain input, or present
an unsupported operation as successful. Common events use protocol-level IDs; provider IDs and exact
frames remain available in ordered `NativeEvent.payload_json`, while `Ready.continuation_id` is the
opaque handle for replacement-process resume.
