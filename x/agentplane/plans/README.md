...
The first live captures preserve complete ordered native frames and the upstream model bodies/chunks.
They omit HTTP headers, cookies, environment variables, credentials, and private user data. The
checked-in replay fixtures retain only the compact upstream bodies/chunks needed by the fake model
server; verbose native logs are regenerated by the capture scripts when needed. Use ordinary
repository secret checks and a small obvious-token guard rather than building a promotion or DLP
subsystem.
