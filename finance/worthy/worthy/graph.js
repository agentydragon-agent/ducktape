...
function handleLoad(e) {
  ...
  const chart = new Chartist.Line(
      '.ct-chart', {series : [ {name : 'series-1', data : data} ]}, {
        axisX : {
          type : Chartist.FixedScaleAxis,
          divisor : 20,
          labelInterpolationFnc : function(
              value) { return moment(value).format('YYYY-MM'); }
        }
      });
}
...
function curfmt(n, sym) {
  return new Intl.NumberFormat('cs-CZ', {style : 'currency', currency : sym})
      .format(n);
}
function assetHtml(c) { return curfmt(c['Amount'], c['Symbol']); }
  ...
  for (let i = 0; i < model['MonthlyTargets'].length; ++i) {
    ...
    for (let j = 0; j < model['YearlyYields'].length; ++j) {
      ...
      if (fiInfo['Reached']) {
        ...
        html += '💰 ≥' +
                curfmt(fiInfo['NeedToLastUntilDeadline'],
                       model['Total']['Symbol']) +
                '<br>' + moment(fiInfo['LastsUntil']).format('YYYY-MM-DD') +
                '<br>' +
