    ...
    return state_history_frame_from_columns(
        {
            ...
            "balance_currency_quanta": currency_quanta_column(state.reshape(-1)),
        },
        CASH_BALANCES_FRAME,
    )
    ...
    return state_history_frame_from_columns(
        {
            ...
            "cost_basis_per_unit_currency_quanta": currency_quanta_column(basis.reshape(-1)),
            ...
            rows.append(
                {
                    ...
                    "mark_currency_quanta": int(channels.mark_currency_quanta[issuer_idx, rollout, month]),
                    ...
                    "forced_recovery_cashout_currency_quanta": int(
                        channels.forced_recovery_cashout_currency_quanta[issuer_idx, rollout, month]
                    ),
                }
            )
    if not rows:
        ...
        rows.append(
            {
                ...
                "eligible_fraction": float(channels.eligible_fractions[issuer_idx, rollout, month]),
                "liquidity_blocked": bool(channels.liquidity_blocked[issuer_idx, rollout, month]),
                "floor_currency_quanta": int(buffers.private_equity_opportunities.floor[month, issuer_idx, rollout]),
                "liquid_net_worth_currency_quanta": int(
                    buffers.private_equity_opportunities.liquid_net_worth[month, issuer_idx, rollout]
                ),
                "shortfall_currency_quanta": int(
                    buffers.private_equity_opportunities.shortfall[month, issuer_idx, rollout]
                ),
                "units_held": float(
                    quantity_column(
                ...
                "proceeds_currency_quanta": int(
                    buffers.private_equity_opportunities.proceeds[month, issuer_idx, rollout]
                ),
            }
        )
...
def _lot_disposition_frame(
    *,
    ...
    return frame_from_columns(
        ...
        cost_basis_consumed_currency_quanta=currency_quanta_column(basis),
        proceeds_currency_quanta=currency_quanta_column(proceeds),
