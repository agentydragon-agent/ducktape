    ...
    return state_history_frame_from_columns(
        {
            ...
            "principal_currency_quanta": currency_quanta_column(principal.reshape(-1)[mask]),
            ...
            "monthly_payment_currency_quanta": currency_quanta_column(
                r_first_view(buffers.state.liability_monthly_payment_state).reshape(-1)[mask]
            ),
            "interest_paid_ytd_currency_quanta": currency_quanta_column(
                r_first_view(buffers.state.liability_interest_ytd_state).reshape(-1)[mask]
            ),
            "principal_paid_ytd_currency_quanta": currency_quanta_column(
...
def decode_mortgage_originations(plan: CompiledSimulation, buffers: SimulationBuffers) -> pl.DataFrame:
    ...
    return frame_from_columns(
        ...
        principal_currency_quanta=currency_quanta_column(plan.liabilities.principal[liabs]),
        ...
        monthly_payment_currency_quanta=currency_quanta_column(plan.liabilities.monthly_payment[liabs]),
    )
...
def decode_mortgage_payments(plan: CompiledSimulation, buffers: SimulationBuffers) -> pl.DataFrame:
    ...
    return frame_from_columns(
        ...
        interest_currency_quanta=currency_quanta_column(
            buffers.properties.mortgage_payment_interest[months, liabs, rollouts]
        ),
        principal_currency_quanta=currency_quanta_column(
            buffers.properties.mortgage_payment_principal[months, liabs, rollouts]
        ),
        total_payment_currency_quanta=currency_quanta_column(
            buffers.properties.mortgage_payment_total[months, liabs, rollouts]
        ),
    )
