          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            <tr>
              ...
              {percentileRows.map(({ percentile, value }, index) => (
                <React.Fragment key={percentile}>
                  {showSelectedColumn && selectedColumnIndex === index && (
                    <td className={SELECTED_COL_CELL}>{fmtCurrencyQuanta(selectedValue, currency)}</td>
                  )}
                  <td className={TABLE_NUMERIC_CELL}>{fmtCurrencyQuanta(value, currency)}</td>
                </React.Fragment>
              ))}
              {showSelectedColumn && selectedColumnIndex === percentileRows.length && (
                <td className={SELECTED_COL_CELL}>{fmtCurrencyQuanta(selectedValue, currency)}</td>
              ...
              {columns.map((column) => (
                ...
                >
                  <div className="font-semibold">
                    {fmtCurrencyQuanta(terminalPercentileValue(column.result, 50), {
                      currencyCode: column.result?.currencyCode,
                      currencyQuantum: column.result?.currencyQuantum,
                    })}
                  </div>
                  <div className="text-[11px] augur-muted">
                    {fmtCurrencyQuanta(terminalPercentileValue(column.result, 5), {
                      currencyCode: column.result?.currencyCode,
                      currencyQuantum: column.result?.currencyQuantum,
                    })}{" "}
                    -{" "}
                    {fmtCurrencyQuanta(terminalPercentileValue(column.result, 95), {
                      currencyCode: column.result?.currencyCode,
                      currencyQuantum: column.result?.currencyQuantum,
                    })}
                  </div>
