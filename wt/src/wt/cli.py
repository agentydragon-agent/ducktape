...
# Register explicit commands for direct CLI invocation (python -m wt.cli <cmd>)
# These delegate to the same handlers used by the sh dispatcher
...
async def cmd_create(
    ctx: typer.Context,
    name: str = typer.Argument(..., help="Worktree name to create"),
    yes: bool = typer.Option(False, "--yes", "-y", help="Skip confirmation prompt"),
) -> None:
    """Create a new worktree."""
    verbose = bool((ctx.obj or {}).get("verbose", False))
    config, _formatter, _daemon_client, _plugin_manager = _create_cli_dependencies(verbose=verbose)
    await handle_create_worktree(
        config, name, CreateWorktreeOptions(from_default=True, from_branch=None, from_worktree=None, confirm=not yes)
    )


@app.command("kill-daemon")
@async_command
async def cmd_kill_daemon(ctx: typer.Context) -> None:
    """Stop the wt daemon."""
    verbose = bool((ctx.obj or {}).get("verbose", False))
    config, _formatter, _daemon_client, _plugin_manager = _create_cli_dependencies(verbose=verbose)
    await handle_kill_daemon(config)
...
@app.command("ls")
@async_command
async def cmd_ls(ctx: typer.Context) -> None:
    """List all worktrees."""
    verbose = bool((ctx.obj or {}).get("verbose", False))
    _config, formatter, daemon_client, _plugin_manager = _create_cli_dependencies(verbose=verbose)
    await handle_list_worktrees(daemon_client, formatter)
...
async def cmd_cp(
    ctx: typer.Context,
    src: str = typer.Argument(..., help="Source worktree name"),
    dest: str = typer.Argument(None, help="Destination worktree name (optional)"),
) -> None:
    """Copy a worktree."""
    verbose = bool((ctx.obj or {}).get("verbose", False))
    config, _formatter, _daemon_client, _plugin_manager = _create_cli_dependencies(verbose=verbose)
    await handle_copy_worktree(config, src, dest)
...
async def cmd_path(
    ctx: typer.Context,
    name: str = typer.Argument(None, help="Worktree name"),
    subpath: str = typer.Argument(None, help="Subpath within worktree"),
) -> None:
    """Get the path to a worktree."""
    verbose = bool((ctx.obj or {}).get("verbose", False))
    config, _formatter, _daemon_client, _plugin_manager = _create_cli_dependencies(verbose=verbose)
    await handle_path_command(config, name, subpath)
...
async def cmd_status(
    ctx: typer.Context,
    name: str = typer.Argument(None, help="Worktree name (optional)"),
) -> None:
    """Show status of worktrees."""
    verbose = bool((ctx.obj or {}).get("verbose", False))
    _config, formatter, daemon_client, _plugin_manager = _create_cli_dependencies(verbose=verbose)
    if name:
        await handle_status_single(daemon_client, formatter, name)
    else:
        await handle_status(daemon_client, formatter)


@app.command("help")
def cmd_help_command() -> None:
