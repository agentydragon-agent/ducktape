...
@pytest.mark.asyncio
async def test_check_habit_status_async_returns_date_object(client):
    """Test that check_habit_status_async returns a date object for the date field."""
    ...
    with patch.object(client.client, "get", return_value=mock_resp):
        ...
        status = await client.check_habit_status_async("test-habit-id", "2025-01-15")
        ...
        status = await client.check_habit_status_async("test-habit-id", test_date)
...
@pytest.mark.asyncio
async def test_set_habit_status_async_returns_date_object(client):
    """Test that set_habit_status_async returns a date object for the date field."""
    # Create a mock response
    mock_resp = AsyncMock(spec=httpx.Response)
    mock_resp.status_code = 200
    mock_resp.raise_for_status.return_value = None
