...
def with_api_key(func: F) -> F:
    ...
    async def wrapper(*args, **kwargs):
        ...
        if not api_key:
            ...
            raise HabitifyError("API key is required. Set HABITIFY_API_KEY environment variable or configure server metadata.")
...
def with_client(func: F) -> F:
    ...
    async def wrapper(*args, **kwargs):
        try:
            ...
            if not api_key:
                from ..habitify_client import HabitifyError
                return create_auth_error("API key is required. Set HABITIFY_API_KEY environment variable or configure server metadata.")
