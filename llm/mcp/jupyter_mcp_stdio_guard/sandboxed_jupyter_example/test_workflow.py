...
def test_network_open_allows_http(tmp_path):
    ...
    composer_yaml = yaml.safe_dump(composer_cfg, sort_keys=False)
    subprocess.run([sys.executable, "-m", "jupyter_mcp_stdio_guard.jupyter_sandbox_compose", "--config", "-"], input=composer_yaml.encode(), check=True, env=env)

    config_dir = bundle_dir / "config"
    kernels_dir = bundle_dir / "kernels"
    p = subprocess.Popen([
        sys.executable, "-m", "jupyter_mcp_stdio_guard.jupyter_mcp_launch",
