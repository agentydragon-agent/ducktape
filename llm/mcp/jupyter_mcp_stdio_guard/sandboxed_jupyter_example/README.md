...
- Two virtualenvs:
  ...
  - Control venv (outside repo): jupyter-server, jupyter-core, jupyter-mcp-server, jupyter_mcp_stdio_guard (editable install)
...
# Editable install of this package so the CLIs are available
~/.venvs/jupyter_control/bin/pip install -e ~/code/ducktape/llm/mcp/jupyter_mcp_stdio_guard
```
...
cat sandboxed_jupyter_example/composer.yaml | \
  ~/.venvs/jupyter_control/bin/python -m jupyter_mcp_stdio_guard.jupyter_sandbox_compose --config -
```
This writes:
...
```json
{
  "mcpServers": {
    "sandboxed_jupyter": {
      ...
      "args": [
        "-m", "jupyter_mcp_stdio_guard.jupyter_mcp_launch",
