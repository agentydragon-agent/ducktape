...
def test_example_bundle_and_launch(tmp_path):
    ...
    subprocess.run(
        [sys.executable, "-m", "adgn_llm.mcp.sandboxed_jupyter_mcp.jupyter_sandbox_compose", "--config", "-"],
    ...
    try:
        ...
        while time.time() < deadline and not tools_resp:
            m = _read_line_json(p.stdout, 1.0)
            if m and m.get("id") == 99 and ("result" in m or "error" in m):
                tools_resp = m
        assert tools_resp and "result" in tools_resp, f"tools/list failed: {tools_resp}"
        ...
        _send_line_json(p.stdin, {
            ...
            "params": {"name": "append_execute_code_cell", "arguments": {"cell_source": code_net}}
        })
        exec_net = None
        deadline = time.time() + 45.0
        ...
        assert ("NET_FAIL:" in blob_net) or ("Error" in blob_net) or ("timed out" in blob_net) or ("Name or service not known" in blob_net) or ("Network is unreachable" in blob_net), blob_net
        ...
        assert ("Permission" in blob) or ("Operation not permitted" in blob) or ("Errno" in blob) or ("Connection was lost" in blob), blob
    finally:
        try:
            p.terminate()
        except Exception:
...
def test_network_open_allows_http(tmp_path):
    ...
    composer_yaml = yaml.safe_dump(composer_cfg, sort_keys=False)
    subprocess.run([sys.executable, "-m", "adgn_llm.mcp.sandboxed_jupyter_mcp.jupyter_sandbox_compose", "--config", "-"], input=composer_yaml.encode(), check=True, env=env)

    config_dir = bundle_dir / "config"
    kernels_dir = bundle_dir / "kernels"
    p = subprocess.Popen([
        sys.executable, "-m", "adgn_llm.mcp.sandboxed_jupyter_mcp.jupyter_mcp_launch",
        "--config", str(config_dir),
        "--kernels", str(kernels_dir),
        "--workspace", str(runtime_dir / "workspace"),
        "--kernel-name", "python3",
        "--port", "0", "--token", "auto", "--start-new-runtime",
        "--log-dir", str(runtime_dir),
    ...
    try:
        _send_line_json(p.stdin, {"jsonrpc": "2.0", "id": 1, "method": "initialize", "params": {"protocolVersion": "2025-06-18", "capabilities": {"tools": {}}, "clientInfo": {"name": "pytest", "version": "0.0.1"}}})
        init = None
        deadline = time.time() + 45.0
        ...
        _send_line_json(p.stdin, {"jsonrpc": "2.0", "id": 2, "method": "tools/call", "params": {"name": "append_execute_code_cell", "arguments": {"cell_source": code_net}}})
        resp = None
        deadline = time.time() + 45.0
