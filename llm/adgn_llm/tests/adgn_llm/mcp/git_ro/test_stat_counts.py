...
async def test_git_ro_stat_counts(tmp_path: Path) -> None:
    ...
    for it in items:
        ...
        if path == "file.txt":
            additions = it.get("additions") if isinstance(it, dict) else getattr(it, "additions", 0)
            deletions = it.get("deletions") if isinstance(it, dict) else getattr(it, "deletions", 0)
            # Expect exactly two additions (two new lines) and zero deletions
            assert int(additions) == 2, f"Expected additions==2 for file.txt, got {additions}"
            assert int(deletions) == 0, f"Expected deletions==0 for file.txt, got {deletions}"
            return
