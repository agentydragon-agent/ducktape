...
def _default_db_path() -> Path:
    ...
    root = base / "adgn-llm" / "openai-responses-db"
...
class ResponsesDB:
    ...
    async def claim_key(self, key: str, model: str, input_obj: Any, kwargs_obj: Any) -> bool:
    ...
    async def get_status(self, key: str) -> Optional[str]:
        assert self._conn is not None, "DB not initialized"
        cur = await self._conn.execute("SELECT status FROM responses WHERE key = ?", (key,))
        row = await cur.fetchone()
        await cur.close()
        return row[0] if row else None

    async def get_complete_response(self, key: str) -> Optional[Dict[str, Any]]:
        """Return full response (summary JSON) if status == 'complete', else None."""
        assert self._conn is not None, "DB not initialized"
        cur = await self._conn.execute("SELECT response_summary_json, status FROM responses WHERE key = ?", (key,))
        row = await cur.fetchone()
        await cur.close()
        if not row:
            return None
    ...
    async def finalize_response(self, key: str, response_obj: Any, summary_obj: Any | None = None) -> None:
        assert self._conn is not None, "DB not initialized"
