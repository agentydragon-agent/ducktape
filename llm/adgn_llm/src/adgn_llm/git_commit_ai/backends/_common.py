...
async def run_subprocess(
    cmd: list[str], *, timeout: Optional[timedelta], debug: bool, logger: logging.Logger
    ...
    try:
        if timeout is None:
            ...
            stdout, stderr = await asyncio.wait_for(proc.communicate(), timeout=timeout.total_seconds())
    except TimeoutError:
        proc.terminate()
        await proc.wait()
        secs_str = "infinite" if timeout is None else str(int(timeout.total_seconds()))
...
def build_prompt_for_claude(repo: Repo, diff: str, passthru: list[str], previous_message: Optional[str]) -> str:
    prompt = _build_prompt_claude(repo, diff, passthru, previous_message)
    return truncate_prompt(prompt)


def build_prompt_for_codex(repo: Repo, include_all: bool, previous_message: Optional[str]) -> str:
