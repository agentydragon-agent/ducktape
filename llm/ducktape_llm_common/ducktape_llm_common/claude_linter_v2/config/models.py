...
class AccessControlRule(BaseModel):
    ...
    message: Optional[str] = Field(None, description="Custom message to show")
...
class PythonConfig(BaseModel):
    ...
    hard_blocks: PythonHardBlock = Field(
        default_factory=PythonHardBlock,
        description="AST-based hard blocks"
    )
    ...
    relaxed_rules_for_tests: List[str] = Field(
        default_factory=lambda: ["bare_except", "type_checking"],
...
class HookConfig(BaseModel):
    ...
    autofix_categories: List[AutofixCategory] = Field(
        ...
        description="Categories to autofix (empty = use defaults)"
    )
    inject_permissions: bool = Field(
        True,
        description="Whether to inject permission info in responses"
    )
...
class LLMAnalysisConfig(BaseModel):
    ...
    check_types: List[str] = Field(
        default_factory=lambda: ["error_hiding", "security_issues"],
...
class TaskProfile(BaseModel):
    ...
    duration: Optional[str] = Field(None, description="How long profile is active")
...
class ClaudeLinterConfig(BaseModel):
    ...
    access_control: List[AccessControlRule] = Field(
        default_factory=list,
        description="Path-based access control rules"
    )
    
    # Repo-wide predicate rules
    repo_rules: List[PredicateRule] = Field(
        default_factory=list,
        description="Repository-wide predicate rules"
    )
    ...
    hooks: Dict[str, HookConfig] = Field(
        default_factory=lambda: {
            ...
            "post": HookConfig(
                auto_fix=True,
                autofix_categories=[AutofixCategory.FORMATTING]
            ),
    ...
    test_patterns: List[str] = Field(
        ...
        description="Global test file patterns"
    )
    
    # LLM analysis
    llm_analysis: LLMAnalysisConfig = Field(
        default_factory=LLMAnalysisConfig,
...
class HookResponse(BaseModel):
    ...
    decision: Optional[HookDecision] = Field(None, description="Explicit decision")
    reason: Optional[str] = Field(None, description="Human-readable reason")
    
    # Optional fields  
    stopReason: Optional[str] = Field(None, description="Message shown when stopping")
    suppressOutput: Optional[bool] = Field(None, description="Hide stdout (default false)")
    
    # Custom fields for our use (will be ignored by Claude Code)
    suggestions: Optional[List[str]] = Field(None, description="Suggestions for fixes")
