...
def create_plot_data_point(iter_data: dict[str, Any], facet_name: str) -> dict[str, Any]:
    """Create a plot data point for a given iteration and facet."""
    if facet_name == "overall":
        stats = iter_data["overall"]
    else:
        stats = iter_data["facets"][facet_name]
...
class ScoreEvolutionPlotter:
    ...
    def generate_plots(self, run_dir: Path) -> tuple[Path, Path]:
        ...
        combined_plot = self._create_combined_plot(df)
        
        # Create faceted plot  
        faceted_plot = self._create_faceted_plot(df)
    ...
    def _create_combined_plot(self, df):
        ...
        from plotnine import (
            aes, geom_errorbar, geom_line, geom_point, ggplot, labs,
            position_dodge, theme, theme_minimal, element_text
        )
    ...
    def _create_faceted_plot(self, df):
        ...
        from plotnine import (
            aes, element_text, facet_wrap, geom_errorbar, geom_line,
            geom_point, ggplot, labs, theme, theme_minimal
        )
...
class ScoreEvolutionReporter:
    ...
    def generate_report(self, run_dir: Path, log_path: Path, plot_paths: tuple[Path, Path] = None) -> str:
...
def analyze_rollout_capacity(log_path: Path, context_limit: int = 200000) -> dict[str, Any]:
...
def print_rollout_analysis_report(log_path: Path, context_limits: list[int] = None) -> None:
