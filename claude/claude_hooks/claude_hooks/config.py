...
class HookConfiguration(BaseModel):
    ...
    timeout_seconds: int = Field(
        default=60, ge=1, le=600, description="Hook execution timeout",
    )
...
class AutofixerConfig(HookConfiguration):
    ...
    tools: list[str] = Field(
        default_factory=lambda: ["Edit", "MultiEdit", "Write"],
        description="List of tool names that trigger autofix",
    )
