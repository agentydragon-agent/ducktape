...
# Note: PROXY_ENV is loaded from proxy_env.bzl (gitignored). The bazelisk wrapper
# copies from proxy_env_default.bzl if it doesn't exist. On Claude Code web,
# the session hook generates proxy_env.bzl with actual proxy settings.
lock(
    name = "requirements",
