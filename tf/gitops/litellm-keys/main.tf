...
# ============================================================================
# The coder agent at public-coder-agent.allegedly.works and its Console-launched
# Codex sandbox are two shells for the same durable Agent identity. They share one
# virtual key for the Codex-subscription and Gemini lineups rather than OpenClaw's
# general key, so the identity remains separately attributable and revocable.
# The key is reflected only into public-coder-agent: OpenClaw may consume it directly,
# while the trusted iron-proxy substitutes it for the Console-launched Codex sandbox's
# non-secret placeholder. Deleting this key is the Agent's provider kill switch.
...
resource "kubernetes_secret" "public_coder_agent" {
  metadata {
    ...
    namespace = "litellm"
    annotations = {
      description                                                     = "LiteLLM virtual key for public-coder-agent OpenClaw and iron-proxy-mediated Haku Console Codex shells (subscription models through CLIProxyAPI, Gemini chat models, plus embeddings)"
      "reflector.v1.k8s.emberstack.com/reflection-allowed"            = "true"
