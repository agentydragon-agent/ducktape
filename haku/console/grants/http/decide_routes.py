...
async def decide_http_request(
    ...
    constraint). The ``Authorization`` bearer is the decision endpoint token; it authenticates
    the decision endpoint but does not identify an Agent. The required session token in the body
    ...
    if not service.authenticate_proxy(authorization):
        raise HTTPException(status_code=401, detail="decision endpoint token was rejected")
    try:
