...
def test_deployed_console_config_is_valid() -> None:
    ...
    assert subjects["haku"].username == "system:serviceaccount:haku-sandbox:haku"
    assert subjects["haku"].groups == (
        "system:serviceaccounts",
        "system:serviceaccounts:haku-sandbox",
...
def test_deployed_console_settings_load_from_the_shared_yaml(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert settings.codex_runtime.api_base_url == "http://litellm.litellm.svc.cluster.local:4000/v1"
