"""The deploy-owned system prompt a chat runtime session is started with.

The template is **deploy config, not code and not haku-state**: it is mounted into the console's
ConfigMap and named by an absolute path in that runtime's configuration. A system prompt
