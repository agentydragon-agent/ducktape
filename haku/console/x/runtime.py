...
class RuntimeRegistry:
    ...
    def __init__(
        ...
        resources: Mapping[RuntimeKind | RuntimeKey | tuple[UUID, RuntimeKind], AgentRuntimeResources] | None = None,
    ):
        ...
        for key, source in (resources or {}).items():
            if isinstance(key, RuntimeKind):
                ...
                continue
            identity = key if isinstance(key, RuntimeKey) else RuntimeKey(agent_id=key[0], runtime_kind=key[1])
