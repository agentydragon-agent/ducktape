...
// frontend bridge, gated by the standing consent grant. Two shapes: a one-shot read
...
// resolves to a discriminated result so the caller reports it over the frontend bridge either way.
...
// Manages shell-side `navigator.geolocation.watchPosition` streams keyed by the frontend bridge's
...
export class GeolocationWatcher {
  private readonly watches = new Map<string, number>(); // frontend bridge watch ID → browser watch ID
  ...
  // the caller can send each a terminal message over the frontend bridge.
  stopAll(): string[] {
