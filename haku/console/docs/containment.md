...
the frontend bridge, and the CSP. **A persistent "trust indicator" is not a security control**: a web
...
1. **The capability gate makes a faked control inert.** A fake "Launch" button can only emit
   a frontend bridge request, which the shell re-gates with exact-Origin admission + confirm + bearer.
2. **The shell is the only trustworthy approval surface.** Immediate frontend bridge escalations render as
...
## The frontend bridge protocol (postMessage)

Display data and operator intent go straight to Haku's backend (it holds the `haku-state`
creds); the frontend bridge carries only actions needing the trusted side. Every inbound message is
...
the shared frontend bridge client also starts a `MutationObserver` over the iframe document's `<head>` and
...
The `<tool-call>` affordance does **not** give the iframe a direct frontend bridge action in v1.
...
- **The shell holds every watch.** `startGeolocationWatch` runs the actual
  ...
  keyed by the frontend bridge request ID; each fix is relayed to the frame. So a prompt-injected Haku can
