...
% endif
<%# CLEANUP(2026-04-08): hasattr() guard is backward-compat for the installed Nix package,
    whose _render_extra_context() only passes (secrets, fork_result, web_mode). Once the
    package is rebuilt with the updated signature that passes profile= and
    bazel_remote_proxy_sock=, drop the hasattr() and use profile.bazel_remote_proxy directly. %>
% if hasattr(profile, 'bazel_remote_proxy') and profile.bazel_remote_proxy and bazel_remote_proxy_sock:
