...
module "lxc_k8s_test_image" {
  source       = "../modules/nixos-lxc-image"
  flake_target = "lxc-k8s-test"
  proxmox_host = var.proxmox_host
  repo_root    = local.repo_root
  nix_dir_hash = local.nix_dir_hash
}
...
# lxc-k8s-test — NixOS LXC container + k8s worker (test)
# K8s credentials placed manually after boot (no cloud-init in LXC).
module "lxc_k8s_test" {
  source = "../modules/proxmox-lxc"

  ct_name          = "lxc-k8s-test"
  ct_id            = 200
  privileged       = true
  vcpus            = 4
  memory_mb        = 8192
  disk_size_gb     = 50
  template_file_id = module.lxc_k8s_test_image.template_file_id
  ...
  depends_on = [module.lxc_k8s_test_image]
}
