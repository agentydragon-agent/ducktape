...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--provider", choices=LABELS, default="vecm", help="Macro rollout provider from augur.model.markets.registry."
    )
...
def run_server(
    *, augur_config: AugurConfig, dist_dir: Path, default_rollout_config_path: Path, argv: list[str] | None = None
    ...
    rollout_provider = _make_provider(args, augur_config, default_rollout_config_path)
    if args.dist_dir:
        dist_dir = Path(args.dist_dir).resolve()
    print(f"serving Augur on http://{args.host}:{args.port}")
    print(f"rollout provider: {rollout_provider.label}")
