...
class BootstrapEnumOption(InternalModel):
    """One entry in a backend-served enum table the controls panel renders."""
...
class BootstrapSourceLink(InternalModel):
    label: str
    url: str


class BootstrapKnobConfig(InternalModel):
    key: str
    label: str
    min: float
    max: float
    ...
    hint: str | None = None
    source_links: list[BootstrapSourceLink] = Field(default_factory=list)


class BootstrapKnobGroup(InternalModel):
    id: str
    label: str
    knobs: list[BootstrapKnobConfig]


class BootstrapPreset(InternalModel):
    id: str
    label: str
    tone: str = "neutral"
    title: str | None = None
...
class PersonalFinanceSnapshot(InternalModel):
    """Personal-finance state the methodology panel renders verbatim."""

    as_of_date: str
    cash_usd: float
    wealthfront_sp500_usd: float
    ibkr_vt_usd: float
    sp500_proxy_portfolio_usd: float
    private_equity_units: int
    private_equity_shareworks_fmv_usd_per_unit: float
    notes: list[str]


class ModelMetadata(InternalModel):
    """Provider-side metadata strings the methodology panel cites."""

    model_form: str
    horizon_start: str
    economic_path_model: str
    active_evidence: str
    active_market_factors: list[str]
    home_evidence: str
