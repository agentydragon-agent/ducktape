...
def test_simple_model_samples_levels_and_events() -> None:
    model = SimpleExogenousModel(
        current_private_equity_price_usd=50.0,
        parameters=SimpleExogenousModelConfig(
...
def test_simple_provider_config_realizes_native_model() -> None:
    config = SimpleExogenousProviderConfig(
    ...
    assert isinstance(model, SimpleExogenousModel)
    assert sampled.metadata["exogenous_model_id"] == "simple_exogenous_model"
