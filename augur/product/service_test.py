...
def test_outside_rent_emits_yearly_re_pegged_obligation() -> None:
    model = CountingExogenousModel()
    product = _service(model)
    scenario = ScenarioKey(
        exogenous_model_id="current_exogenous_model",
    ...
    assert "rent:location_a" in model.sample_requests[0].required_level_series
...
def test_outside_rent_zero_omits_rent_series_requirement() -> None:
    model = CountingExogenousModel()
    product = _service(model)
    ...
    assert not any("rent:" in series_id for series_id in model.sample_requests[0].required_level_series)


def test_outside_rent_rejects_unknown_location() -> None:
    model = CountingExogenousModel()
    product = _service(model)
    scenario = ScenarioKey(
        exogenous_model_id="current_exogenous_model",
