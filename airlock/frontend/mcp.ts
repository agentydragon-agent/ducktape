...
export class AirlockMcpClient {
  ...
  static async connect(): Promise<AirlockMcpClient> {
    ...
    return new AirlockMcpClient(client);
  }
...
let _clientPromise: Promise<AirlockMcpClient> | null = null;

/** Lazily connect to the MCP endpoint; returns the same instance on repeated calls. */
export function getMcpClient(): Promise<AirlockMcpClient> {
  if (!_clientPromise) _clientPromise = AirlockMcpClient.connect();
