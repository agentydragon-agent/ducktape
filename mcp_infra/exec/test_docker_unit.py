...
async def test_exec_stdout_stderr_timeout(exec_client) -> None:
    # stdout
    r1 = await exec_client.exec(ExecInput.create(["/bin/echo", "hello"], timeout_ms=5000))
    ...
    r2 = await exec_client.exec(ExecInput.create(["sh", "-lc", "echo err 1>&2; exit 3"], timeout_ms=5000))
    assert r2.exit == Exited(exit_code=3)
    assert "err" in (r2.stderr or "")
    # timeout
    r3 = await exec_client.exec(ExecInput.create(["sh", "-lc", "sleep 5"], timeout_ms=500))
...
async def test_persession_exec_timeout_then_next_ok(exec_client) -> None:
    # Force timeout
    t1 = await exec_client.exec(ExecInput.create(["sh", "-lc", "sleep 3"], timeout_ms=500))
    assert t1.exit == TimedOut()
    # Next call should succeed after restart
    r1 = await exec_client.exec(ExecInput.create(["/bin/echo", "ok"], timeout_ms=5000))
    assert r1.exit == Exited(exit_code=0)
    assert isinstance(r1.stdout, str)  # Short output should not be truncated
    assert r1.stdout == "ok\n"
