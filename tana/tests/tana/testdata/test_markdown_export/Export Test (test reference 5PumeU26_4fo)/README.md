Export Test (test reference 5PumeU26_4fo)

- [Daily notes](<Daily notes/README.md>)
- Node content formatting
  - Basic formatting (test reference r1shM2RHNgCv)
    - This is Bold
    - This is Italic
    - This is Highlit
    - This is Underscored
    - This is Strikethrough
  - Datetime (test reference 2Ap-6LC3fVuq)
    - Date: _2025-06-12_
    - Datetime: _2025-06-12T12:38:03[America/Los_Angeles]_
    - Datetime range: _2025-06-12T12:38:11[America/Los_Angeles]/2025-06-12T13:38:12[America/Los_Angeles]_
  - Code (test reference YbPcBamWZFGV)
    - Inline code: foo = "bar"
    - Code block, Python:
      - ```Python

        ```

# !/usr/bin/python

print("hello world")

```
  - Checkbox nodes (test reference HhxGuymRRqrR)
    - [x] Filled with bold
    - [ ] Unfilled with italic
    - [ ] Unfilled reference to filled with bold: [x] Filled with bold
- Inline references (test reference 6aoZJeWmOXcl)
  - Node X
  - Inline reference to Node X: Node X
  - Inline reference to Node X with an alternate alias: Different name for node X
- Links (test reference KhlJy8yJ37KN)
  - Basic link
    - [https://www.google.com/](https://www.google.com/)
  - Inline link: [https://www.reddit.com/](https://www.reddit.com/)
  - Inline link with different text: [here](https://xkcd.com/)
  - Link with embedded additional content:
    - [https://xkcd.com/](https://xkcd.com/)
- [The node below has a supertag applied, "recipe" (test reference hiHpuPTowhDs)](<The node below has a supertag applied, -recipe- (test reference hiHpuPTowhDs)/README.md>)
- [Some other nodes with #recipe supertag: (test reference u00FQD8V08fy)](<Some other nodes with -recipe supertag- (test reference u00FQD8V08fy)/README.md>)
- [Node with multiple supertags: (test reference x2-AdByI7b-a)](<Node with multiple supertags- (test reference x2-AdByI7b-a)/README.md>)
- [Node with supertags with space in name: (test reference x2-AdByI7b-a)](<Node with supertags with space in name- (test reference x2-AdByI7b-a)/README.md>)
- Image (test reference Vi0w332Hvh9b)
  - Plain image
    - ![](https://firebasestorage.googleapis.com/v0/b/tagr-prod.appspot.com/o/notespace%2Fagentydragon%40gmail.com%2Fuploads%2F2025-06-13T00%3A51%3A03.687Z-image.png?alt=media&token=fce13347-79df-42cd-8fd4-eecc10ffa7ba)
  - Image with caption
    - ![Caption of this image - bold, italic](https://firebasestorage.googleapis.com/v0/b/tagr-prod.appspot.com/o/notespace%2Fagentydragon%40gmail.com%2Fuploads%2F2025-06-13T00%3A51%3A49.350Z-image.png?alt=media&token=e5f17b31-9ea5-4f95-b77d-caa4df8c9fd1)
- Search node looking for all #recipe nodes (test reference LkFZLMTHylYl)
- Search node rendered as a table (test reference b9AZZEuj42vC)
- Node rendered as a table (test reference SGrUB4iyWYde)
  - Table
    - Row 1
      - **Standard column**: A1
      - **Number column**: 11
      - Row 1 content foo
      - Row 1 content bar
    - Row 2
      - **Standard column**: A2
      - **Number column**: 22
      - Row 2 content foo
      - Row 2 content bar
    - Row 3
      - **Standard column**: A3
      - **Number column**: 33
      - Row 3 content foo
      - Row 3 content bar
- Table with contextual column (test reference 5PumeU26_4fo)
  - Perfect Eggs (test reference) #recipe
  - Steak #recipe
  - Scramble #recipe
```
