...
Kubernetes and HTTP grants use one shared applicability-subject vocabulary even
though their capability matchers remain separate typed domains. Four concepts
must not share one field:

1. **Source** is the immutable `source_tool_call_id`. It proves manual approval,
   provides idempotency and supports source-set revocation. It is not the later
   request's authorization subject.
2. **Requester** is reconstructed from the source ToolCall's durable principal,
   including its credential binding and optional exact session. Request JSON
   never supplies this identity.
3. **Owner** is the Agent that may list, inspect, release and audit the grant.
   Current Agent-created grants are owned by their requesting Agent. Operator
   inspection and revocation remain separate authority.
4. **Applicability subject** is the only principal tested when authorizing a
   later request.

The shared discriminated subject model has these variants:
...
An Agent subject covers every authenticated execution of that Agent. A session
subject covers only the exact live Console session; a static Agent credential or
replacement session cannot exercise it. An access-profile subject covers an
authenticated context whose trusted, pinned profile ID matches. Missing session
or profile identity fails closed. A session request may still use a broader
Agent or access-profile grant that covers it.

The Agent-facing Kubernetes and HTTP creation tools accept only an applicability
selector such as `applies_to: "agent" | "session"`, defaulting to `agent` to
preserve the implemented Kubernetes behavior. They never accept an arbitrary
Agent or session ID. Console resolves `agent` from the source binding and
`session` from the source ToolCall principal, rejecting `session` when the call
has no exact session. All exact grants created by one ToolCall share its subject
and lifetime.

Access-profile grants are reserved in the shared model but are not Agent-
creatable initially. Standing policy already supplies deploy-managed profile-
wide authority. A future temporary profile-wide grant needs a separate,
conspicuously broad operator surface and validation against the deployment's
configured profiles; it must not arise merely because one Agent requested a
grant while using that profile.

The first implementation shares this subject model, subject-resolution code and
conformance tests, but keeps separate Kubernetes and HTTP grant tables and
services. This avoids prematurely forcing Kubernetes rules, HTTP origins and
credential-use capabilities into one generic matcher or persistence envelope.
A common lifecycle envelope can be extracted after those domains demonstrate
that their remaining ownership and lifecycle fields are genuinely identical.

The shared Python vocabulary lives in `haku/console/grant_subject.py`. It
consists of a strict discriminated `CapabilityGrantSubject`, a trusted
`GrantApplicabilityContext` carrying `agent_id`, optional `session_id` and
optional `access_profile_id`, and one conservative applicability predicate. Domain
services accept only that complete context; no agent-only matching overload
remains after migration. Each domain table stores `owner_agent_id` separately
from a non-null typed JSONB `subject`. The Kubernetes migration renames the
current overloaded `agent_id`, backfills every existing grant with an Agent
subject and preserves its grant ID, source ToolCall, status and timestamps. The
HTTP table starts with the same owner/subject shape.
...
fails closed after the session ends; lifecycle cleanup should also terminalize
remaining active session grants with an audit reason such as `subject_ended`.
...
1. Land `haku/console/grant_subject.py` with the shared strict grant-subject
   model, trusted applicability context and behavioral contract tests.
2. Preserve binding and optional session identity through trusted in-process MCP
   execution.
3. Migrate Kubernetes grants from overloaded `agent_id` to
   `owner_agent_id` plus an explicit subject, backfilling current rows as Agent
   subjects.
4. Change Kubernetes repository and service APIs from Agent-only lookup to
   complete applicability-context matching; add `applies_to` to its creation
   tool and session-end cleanup.
...
17. Run end-to-end grant, subject, credential-selection, failure and rebinding
    tests.
...
- existing Kubernetes grants migrate to explicit Agent subjects without a
  coverage change;
- an Agent subject covers another live session of the same Agent;
- a session subject covers only the exact live source session and stops being
  usable when that session ends;
...
- access-profile subjects are rejected by Agent-facing creation tools;
