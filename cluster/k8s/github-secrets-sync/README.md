...
| Secrets        | Read/write | `tf/gitops/github-secrets-sync` manages the `SOPS_AGE_KEY` GitHub Actions secret for `ducktape` and `gaffer-private`, plus the narrow `BUILDBUDDY_API_KEY` secret for `ducktape` build/test workflows.                                                                          |
| Environments   | Read/write | `tf/gitops/github-secrets-sync` manages the protected `fork-ci-review` environment and its environment-scoped `SOPS_AGE_KEY` for explicitly approved non-agent fork revisions.                                                                                                  |
