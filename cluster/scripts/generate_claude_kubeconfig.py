...
def generate(root: Path) -> None:
    ...
    if not hook_config:
        raise RuntimeError(f"Config file not found under {root}")
    if not hook_config.k8s:
        raise RuntimeError("No k8s config found in hook config")

    kubeconfig_path = os.environ.get("KUBECONFIG")
    if not kubeconfig_path:
        raise RuntimeError("KUBECONFIG not set — run from cluster/ with direnv or set it manually")
