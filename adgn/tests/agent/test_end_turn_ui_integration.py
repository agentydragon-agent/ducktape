...
from adgn.agent.ui.protocol import ToolCall, FunctionCallOutput, UiEndTurnEvt
from adgn.agent.ui.state import new_state
from adgn.agent.ui.reducer import reduce_ui_state
from adgn.agent.ui.shared_bus import UiBus, UiEndTurn
...
def test_end_turn_tool_filtering():
    ...
    assert len(after_output.items) == 0, (
        "end_turn tool output should not create UI items"
    )
