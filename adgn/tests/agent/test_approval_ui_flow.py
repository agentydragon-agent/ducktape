...
from adgn.agent.approvals import ApprovalPolicyEngine, ApprovalHub
from adgn.agent.mcp_manager import McpManager
from adgn.agent.ui import protocol
...
def test_approval_prompt_auto_appears(
    ...
    )  # Uses default policy that asks for approval
    ...
    async def _run_test():
        async with McpManager(specs) as mcp_mgr:
            agent = await MiniCodex.create(
                ...
                handlers=[
                    UiModeHandler(
                        bus=bus, poll_notifications=mcp_mgr.poll_notifications
                    )
                ],
            ...
            with client_ws.websocket_connect("/ws") as ws:
                ...
                for _ in range(50):  # Give more iterations for the approval flow
                    ...
                    if payload.type == "approval_pending":
                        ...
                        print(
                            f"✅ Approval prompt appeared automatically! call_id={approval_call_id}"
                        )
                ...
                assert saw_approval_pending, (
                    "❌ Approval prompt did not appear automatically"
                )
