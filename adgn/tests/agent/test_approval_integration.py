...
from adgn.mcp.approval_policy.engine import PENDING_CALLS_URI, CallDecision, PendingCallsResponse
    ...
    async with make_pg_compositor(servers, policy_engine=engine) as (mcp_client, policy_engine):
        ...
        async with Client(policy_engine.reader) as reader_client:
            ...
            for _ in range(20):  # up to ~1s
                pending_data = await read_text_json_typed(reader_client, PENDING_CALLS_URI, PendingCallsResponse)
