...
def token_mapping_file(tmp_path: Path) -> Path:
    ...
    tokens_file.write_text(
        json.dumps({"test-token-1": "agent-1", "test-token-2": "agent-2"}),
        encoding="utf-8",
    )
    return tokens_file
...
async def test_mcp_server_requires_token_auth(
    ...
    mcp_app = await create_mcp_server_app(
...
@pytest.mark.asyncio
async def test_management_ui_no_auth_required(
    infrastructure_registry: InfrastructureRegistry,
):
...
@pytest.mark.asyncio
async def test_websocket_channels_available_on_ui_server(
    infrastructure_registry: InfrastructureRegistry,
):
    ...
    routes = [route.path for route in ui_app.routes]
...
@pytest.mark.asyncio
async def test_infrastructure_registry_caches_per_agent(
    infrastructure_registry: InfrastructureRegistry,
):
...
@pytest.mark.asyncio
async def test_infrastructure_registry_get_nonexistent(
    infrastructure_registry: InfrastructureRegistry,
):
