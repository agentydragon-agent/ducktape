...
async def test_hydrated_copy_only_exposes_scoped_file_for_local_specimen(specimens_registry) -> None:
    ...
    """
    async with specimens_registry.load_and_hydrate("misc/2025-08-29-pyright_watch_report") as hydrated:
...
async def test_hydrated_copy_git_specimen_has_wt_tree_rooted_correctly(specimens_registry) -> None:
    ...
    """
    async with specimens_registry.load_and_hydrate("ducktape/2025-11-20-01") as hydrated:
        assert hydrated.content_root.is_dir(), f"hydrated content root not a directory: {hydrated.content_root}"
