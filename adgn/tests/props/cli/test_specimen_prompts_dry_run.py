...
from adgn.props.cli.main import app
...
def test_run_specimen_dry_run_renders(allow_general):
    ...
    argv = ["run", SPECIMEN_NAME, "--dry-run"]
...
def test_run_specimen_discover_dry_run_renders():
    runner = CliRunner()
    result = runner.invoke(app, ["run", SPECIMEN_NAME, "--preset", "discover", "--dry-run"])
    assert result.exit_code == 0, result.output
    out = result.output
    saved = _extract_saved_prompt_path(out)
    text = _read(saved)
...
def test_run_specimen_grade_dry_run_renders(tmp_path: Path):
    ...
    runner = CliRunner()
    result = runner.invoke(app, ["run", SPECIMEN_NAME, "--preset", "find", "--dry-run"])
    assert result.exit_code == 0, result.output
    out = result.output
    saved = _extract_saved_prompt_path(out)
    text = _read(saved)
