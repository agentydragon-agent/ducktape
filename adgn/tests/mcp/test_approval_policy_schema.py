...
async def test_approval_policy_tool_schemas():
    ...
    async with McpManager(specs) as mcp:
        ...
        assert "source" in props, (
            f"Missing 'source' parameter. Got: {list(props.keys())}"
        )
        assert "rationale" in props, (
            f"Missing 'rationale' parameter. Got: {list(props.keys())}"
        )
        ...
        if "a" in props and "kw" in props:
            assert False, (
                f"Withdraw tool has generic wrapper arguments: {list(props.keys())}"
            )

        assert "proposal_id" in props, (
            f"Missing 'proposal_id' parameter. Got: {list(props.keys())}"
        )
        ...
        if "a" in props and "kw" in props:
            assert False, (
                f"get_status tool has generic wrapper arguments: {list(props.keys())}"
            )
