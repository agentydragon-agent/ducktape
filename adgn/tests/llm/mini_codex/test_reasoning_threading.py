...
async def test_reasoning_threading_filters_reasoning_from_next_input(
    ...
    ri1_idx = next((i for i, item in enumerate(turn2_input)
                    if isinstance(item, ReasoningItem) and item.id == "rs_turn1"), None)
    ...
    ri1_idx = next((i for i, item in enumerate(turn3_input)
                    if isinstance(item, ReasoningItem) and item.id == "rs_turn1"), None)
    ...
    ri2_idx = next((i for i, item in enumerate(turn3_input)
                    if isinstance(item, ReasoningItem) and item.id == "rs_turn2"), None)
