...
from adgn.llm.sandboxer import (  # type: ignore[attr-defined]
    FSConfig,
    NetConfig,
    PlatformConfig,
    Policy,
...
def test_fs_write_paths_expand_to_params_and_dirs(tmp_path):
    ...
    assert defs["WP_0"].startswith(str(tmp_path))
    assert Path(defs["WP_0"]).is_absolute()
    assert defs["WP_1"].startswith(str(tmp_path))
    assert Path(defs["WP_1"]).is_absolute()
...
def test_fs_read_paths_expand_to_params_and_dirs(tmp_path):
    ...
    assert defs["RP_0"].startswith(str(tmp_path))
    assert Path(defs["RP_0"]).is_absolute()
    assert defs["RP_1"].startswith(str(tmp_path))
    assert Path(defs["RP_1"]).is_absolute()
