...
def require_macos_rtc():
    ...
    if (
        importlib.util.find_spec("jupyter_collaboration") is None
        or importlib.util.find_spec("pycrdt") is None
    ):
        pytest.skip(
...
def pytest_runtest_makereport(item, call):
    ...
    try:
        for p in sorted(Path("/tmp").glob("sjmcp-*/mcp_*.log"))[-6:]:
...
def collect_mcp_logs_fn():
    def _collect() -> tuple[str, str]:
        out = err = ""
        try:
            for p in sorted(Path("/tmp").glob("sjmcp-*/mcp_stdout.log"))[-3:]:
                try:
                    ...
                    pass
            for p in sorted(Path("/tmp").glob("sjmcp-*/mcp_stderr.log"))[-3:]:
                try:
                    with p.open("rb") as fh:
                        err += (
                            f"\n== {p} ==\n"
...
def mcp_stdio_protocol(send_line_json_fn, read_line_json_fn):
    ...
    def _protocol(
        ...
        log_out = (
            (Path(artifacts) / "mcp_protocol_out.log").open("ab") if artifacts else None
        )
        log_in = (
            (Path(artifacts) / "mcp_protocol_in.log").open("ab") if artifacts else None
        )
        try:
            init = {
                "jsonrpc": "2.0",
            ...
            assert resp, f"initialize failed: {resp}"
            assert resp.get("id") == 1, f"initialize missing id: {resp}"
            ...
            assert resp2, f"tool call failed: {resp2}"
            assert resp2.get("id") == 2, f"tool call missing id: {resp2}"
            assert "result" in resp2, f"tool call missing result: {resp2}"
            return resp2["result"]
        finally:
