...
def _build_wrapper_cmd(ws: Path, run_root: Path, port: int) -> list[str]:
    ...
    return [
        sys.executable,
        "-m",
        "adgn.llm.mcp.sandboxed_jupyter_mcp.wrapper",
        "stdio",
        "--policy-config",
...
def test_network_modes_http_boundary(
    ...
    with launch_proc(cmd, env_update=pkg_src_env_update) as proc:
        ...
        blob = str(result)
        if expect_http:
            if "NET_OK:" not in blob and "Example Domain" not in blob:
                pytest.fail(blob)
        else:
            assert "NET_FAIL:" in blob, blob
            assert "NET_OK:" not in blob, blob
...
def test_env_set_and_passthrough_visible_in_kernel(
    ...
    with launch_proc(cmd, env_update=env_update) as proc:
        ...
        blob = str(result)
        assert "ENV_FOO_SET= SET_OK" in blob, blob
        assert "ENV_BAR_PASS= PASS_OK" in blob, blob
...
def test_fs_read_allow_and_deny(
    ...
    with launch_proc(cmd, env_update=pkg_src_env_update) as proc:
        ...
        blob = str(result)
        assert "READ_OK= OK_FILE" in blob, blob
        assert "READ_DENY=" in blob, blob
        assert "READ_DENY_OOPS" not in blob, blob
