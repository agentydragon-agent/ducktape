...
- [ ] Create `AgentID` type with Pydantic validation in `agent/types.py`
- [ ] Create `mcp_bridge/` module structure
- [ ] Implement `load_tokens()` in `mcp_bridge/auth.py`
- [ ] Implement `TokenRoutingASGI` in `mcp_bridge/auth.py`

#### 5a.2: Infrastructure Registry
- [ ] Create `InfrastructureRegistry` class in `mcp_bridge/server.py`
...
- [ ] Create `mcp_bridge/servers/agents.py`
  - `list` resource (all agents with state)
  - `presets` resource
  - `create_agent` tool
  - `delete_agent` tool
  - `boot_agent` tool
- [ ] Wire up resource notifications on agent state changes

#### 5a.4: Dual Compositor in AgentContainer
- [ ] Add `_user_compositor` to `AgentContainer`
- [ ] Modify `_setup_compositors()` to create both:
  - Agent compositor (existing + policy gateway)
  - User compositor (reader, admin, agent_control for internal)
- [ ] Add `external: bool` flag to control `agent_control` mounting
- [ ] Add `user_compositor` property

#### 5a.5: Agent Control Server
- [ ] Create `mcp_bridge/servers/agent_control.py`
  - `send_prompt` tool
  - `abort_run` tool
- [ ] Wire to container methods

#### 5a.6: Update app.py
- [ ] New lifespan with token loading + external agent startup
- [ ] Replace current MCP setup with `TokenRoutingASGI`
- [ ] Keep REST API endpoints (frontend still uses them)

#### 5a.7: Integration Test
- [ ] Test: user token → global compositor → sees agents
- [ ] Test: agent token → agent compositor → gated tools
- [ ] Test: external agent startup from tokens config
