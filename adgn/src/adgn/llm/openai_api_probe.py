    ...
    def _std_lat(obj) -> float | None:
        if isinstance(obj, ProbeRun):
            vals: list[float] = [
                float(c.latency_s)
                for c in obj.calls
                if c.ok and c.latency_s is not None
            ]
            if len(vals) >= 2:
                m: float = sum(vals) / len(vals)
                var_val: float = sum((x - m) ** 2 for x in vals) / len(vals)
                std_dev: float = var_val**0.5
                return std_dev
...
async def main() -> None:
    ...
    responses_spec = ProbeSpec(
        name="responses",
        create=_create_responses,
        snippet=_snippet_from_responses,
    )
    ...
    with Live(console=console, refresh_per_second=4) as live:
        for fut in asyncio.as_completed(tasks):
            ...
            # Single table for partial arrivals (one side waiting)
            partials_table = make_results_table(
                title="Partial arrivals (one side waiting)",
                header_style="bold magenta",
                model_ratio=3,
            )

            # Collect mids with exactly one side arrived
            partial_mids: list[str] = []
            for mid, pair in acc.items():
                has_r = len(pair["responses"]) > 0
                has_c = len(pair["chat"]) > 0
                if has_r ^ has_c:  # exactly one side
                    partial_mids.append(mid)

            def partial_sort_key(mid: str):
                # Use whichever side has calls for latency; fall back to inf
                lat_candidates: list[float] = []
                if acc[mid]["responses"]:
                    lat = _avg_lat(ProbeRun(model_id=mid, calls=acc[mid]["responses"]))
                    if lat is not None:
                        lat_candidates.append(lat)
                if acc[mid]["chat"]:
                    lat = _avg_lat(ProbeRun(model_id=mid, calls=acc[mid]["chat"]))
                    if lat is not None:
                        lat_candidates.append(lat)
                lat_val = min(lat_candidates) if lat_candidates else inf
                return (priority_index(mid), lat_val, mid)

            partial_mids.sort(key=partial_sort_key)

            for mid, end_section in iter_with_break(
                partial_mids,
                lambda x: classify(x),
            ):
                rcell = cell_for_kind(mid, "responses")
                ccell = cell_for_kind(mid, "chat")
                partials_table.add_row(mid, rcell, ccell, end_section=end_section)
            ...
            live.update(Group(new_table, partials_table, others_table))
