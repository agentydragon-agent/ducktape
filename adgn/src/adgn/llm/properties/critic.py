...
from adgn.llm.properties.models.issue import IssueId, Occurrence, LineRange
    ...
    *,
    ...
    def _ensure_work_payload() -> CriticSubmitPayload:
        if state.work is None:
            state.work = CriticSubmitPayload()
        return cast(CriticSubmitPayload, state.work)
    ...
    async def submit(payload: SubmitInput) -> dict[str, Any]:
        ...
        """
        if (
            getattr(state, "result", None) is not None
