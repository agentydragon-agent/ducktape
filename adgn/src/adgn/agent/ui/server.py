...
class AgentSession:
    ...
    def __init__(
        self, manager: ConnectionManager, approval_hub: ApprovalHub | None = None
    ) -> None:
    ...
    def build_snapshot(self, sampling=None) -> Snapshot:
        ...
        if self.approval_engine:
            ...
            proposals = [
                ProposalInfo(
                    id=p.id, status=p.status, rationale=p.rationale, source=p.source
                )
...
def create_app(*, require_static_assets: bool = True) -> FastAPI:
    ...
    # Create a session immediately so tests can use app.state.session without awaiting startup
    approval_hub0 = getattr(app.state, "approval_hub", None)
    app.state.session = AgentSession(manager, approval_hub=approval_hub0)
    # Note: a fresh session will also be created during startup; keeping this allows pre-start usage
        ...
        try:
            while True:
                ...
                if isinstance(im, ApproveIn):
                    ...
                    await manager.send_payload(
                        ApprovalDecisionEvt(
                            call_id=im.call_id, decision=ApprovalApprove()
                        )
                    )
                    continue
                ...
                if isinstance(im, DenyContinueIn):
                    ...
                    await manager.send_payload(
                        ApprovalDecisionEvt(
                            call_id=im.call_id, decision=ApprovalDenyContinue()
                        )
                    )
                    continue
                ...
                if isinstance(im, DenyIn):
                    ...
                    await manager.send_payload(
                        ApprovalDecisionEvt(
                            call_id=im.call_id, decision=ApprovalDenyAbort()
                        )
                    )
                    continue
                    ...
                    if session.approval_engine:
                        try:
                            ...
                            await manager.send_payload(
                                ErrorEvt(
                                    code=ErrorCode.INVALID_COMMAND,
                                    message=f"Proposal not found: {im.proposal_id}",
                                )
                            )
                    continue
...
def run_uvicorn(host: str = "127.0.0.1", port: int = 8765) -> None:
    uvicorn.run(
        "adgn.agent.ui.server:create_app",
        host=host,
        port=port,
        log_level="info",
        factory=True,
