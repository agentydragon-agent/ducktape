...
class PerFileRunResult(BaseModel):
    ...
    issues_in_file: set[BaseIssueID] = Field(description="Issue IDs that have at least one occurrence in this file")
    critique: CriticSubmitPayload | None = Field(
        default=None, description="Critique payload from agent (None if failed)"
    )
    grade: GradeSubmitInput | None = Field(
        default=None,
        description="Grading result comparing critique to canonical issues in this file (None if critique failed)",
    )
    detected_issue_ids: set[BaseIssueID] = Field(
        default_factory=set, description="Issue IDs that were successfully detected (matched as TP by grader)"
    )
...
class PerIssueResult(BaseModel):
    ...
    files_containing_issue: set[str] = Field(description="Files where this issue has at least one occurrence")
    files_that_detected: set[str] = Field(
        default_factory=set, description="Files where the per-file critic run successfully detected this issue"
    )
...
class FileIssuesIndex:
    ...
    file_to_issues: dict[str, set[str]]  # file path -> issue IDs
    issue_to_files: dict[str, set[str]]  # issue ID -> file paths (single-file: one element)
    multi_file_issues: dict[str, set[str]]  # issue ID -> file paths (multi-file issues)
...
def build_file_issues_index(issues: dict[str, IssueRecord]) -> FileIssuesIndex:
    ...
    multi_file_issues: dict[str, set[str]] = {}
    ...
    for issue_id, issue_rec in issues.items():
        ...
        for occ in issue_rec.instances:
            for file_path in occ.files:
                files_for_issue.add(str(file_path))

        # Only index single-file issues in the main index
        if len(files_for_issue) == 1:
            file_path_str = next(iter(files_for_issue))
            file_to_issues.setdefault(file_path_str, set()).add(issue_id)
            issue_to_files[issue_id] = files_for_issue
        else:
            # Track multi-file issues separately
            multi_file_issues[issue_id] = files_for_issue

    return FileIssuesIndex(
        file_to_issues=file_to_issues, issue_to_files=issue_to_files, multi_file_issues=multi_file_issues
    )
...
async def run_per_file_eval(
    ...
    *,
    ...
    """
    async with SpecimenRegistry.load_and_hydrate(specimen, gitconfig) as (rec, content_root):
        ...
        async def process_one_file(file_path: str) -> PerFileRunResult:
            ...
            logger.info(f"Running critic on {file_path} (issues: {', '.join(sorted(issues_in_file))})")
            ...
            # Filter FPs to only those that touch this file
            filtered_fps = {
                k: v for k, v in rec.false_positives.items() if any(file_path in str(occ.files) for occ in v.instances)
            }
            ...
            # Extract covered TP IDs from coverage data
            detected_ids = {tp_id for tp_id, cov in grade.canonical_tp_coverage.items() if cov.covered_by}
        ...
        file_precisions = [run.grade.reported_issue_ratios.tp for run in per_file_runs if run.grade is not None]
        file_recalls = [run.grade.recall for run in per_file_runs if run.grade is not None]
