...
the clustering agent's RLS-scoped credentials. The clustering_run_id is automatically
fetched via the current_clustering_run_id() database function.
...
def _get_current_clustering_run_id(session) -> int:
    """Get the current clustering run ID from the database.

    Uses the PostgreSQL current_clustering_run_id() function which extracts
    the ID from the database username (clustering_agent_{run_id} pattern).
    ...
    Raises:
        RuntimeError: If not connected as clustering agent
    """
    result = session.execute(text("SELECT current_clustering_run_id()"))
    clustering_run_id = result.scalar()
    if clustering_run_id is None:
        raise RuntimeError("Not connected as clustering agent (current_clustering_run_id() returned NULL)")
    return int(clustering_run_id)
...
def create_cluster(cluster_name: str, description: str) -> int:
    ...
    Example:
        from adgn.props.clustering.helpers import create_cluster
    ...
    Note:
        The clustering_run_id is automatically fetched from the database using
        current_clustering_run_id(). You must be connected as a clustering agent
        user (clustering_agent_{run_id}).
    """
    with get_session() as session:
        clustering_run_id = _get_current_clustering_run_id(session)
        cluster = UnknownCluster(
            clustering_run_id=clustering_run_id, cluster_name=cluster_name, description=description
        )
        session.add(cluster)
...
def assign_to_cluster(grader_run_id: str, unknown_id: str, cluster_name: str, rationale: str) -> None:
    ...
    Example:
        from adgn.props.clustering.helpers import create_cluster, assign_to_cluster
    ...
    with get_session() as session:
        ...
        cluster = session.execute(
            select(UnknownCluster).where(
                UnknownCluster.clustering_run_id == clustering_run_id, UnknownCluster.cluster_name == cluster_name
            )
        ).scalar_one_or_none()
...
def assign_to_existing_tp(grader_run_id: str, unknown_id: str, tp_id: str, rationale: str) -> None:
    ...
    Example:
        from adgn.props.clustering.helpers import assign_to_existing_tp
...
def assign_to_existing_fp(grader_run_id: str, unknown_id: str, fp_id: str, rationale: str) -> None:
    ...
    Example:
        from adgn.props.clustering.helpers import assign_to_existing_fp
...
def cancel_assignment(grader_run_id: str, unknown_id: str, cancellation_reason: str) -> None:
    ...
    Example:
        from adgn.props.clustering.helpers import cancel_assignment, assign_to_cluster
    ...
    with get_session() as session:
        ...
        assignment = session.execute(
            select(UnknownAssignment).where(
                UnknownAssignment.clustering_run_id == clustering_run_id,
