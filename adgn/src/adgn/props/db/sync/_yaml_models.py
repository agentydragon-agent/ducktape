...
class YAMLOccurrence(BaseModel):
    ...
    files: dict[str, int | list[int] | list[list[int]] | None] = Field(
        description="File paths to line specifications (supports multiple shapes)"
    )
    ...
    def to_tp_occurrence(self) -> TruePositiveOccurrence:
        ...
        Converts string paths to Path objects and normalized ranges to LineRange objects.
        """
        ...
        for file_str, ranges_val in self.files.items():
            if ranges_val is None:
                ...
                # After normalization, ranges_val is list[list[int]]
                # Type narrowing: we know it's not None, int, or list[int] anymore
                ranges: list[list[int]] = ranges_val  # type: ignore[assignment]
                files_dict[Path(file_str)] = [LineRange(start_line=r[0], end_line=r[1]) for r in ranges]
