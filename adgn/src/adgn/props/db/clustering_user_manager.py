...
class ClusteringUserManager(TempUserManager):
    ...
    read reference tables (snapshots, TPs, FPs, grader_runs, critiques).
    ...
    async def grant_permissions(self, username: str) -> None:
        ...
        - Read-only on reference tables (snapshots, TPs, FPs, grader_runs, critiques, critic_runs)
        ...
        async with self.admin_engine.begin() as conn:
            ...
            for table in clustering_tables:
                await conn.execute(text(f"GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE {table} TO {username}"))
            ...
            await conn.execute(text(f"GRANT USAGE ON ALL SEQUENCES IN SCHEMA public TO {username}"))
