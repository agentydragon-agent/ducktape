...
def hash_and_upsert_prompt(
    ...
    improvement_run_id: UUID | None = None,
    ...
    Args:
        ...
        improvement_run_id: Optional ID of the improvement run that generated this prompt
    ...
    with get_session() as session:
        _upsert_prompt_with_session(
            session, prompt_sha256, prompt_text, prompt_optimization_run_id, improvement_run_id, template_file_path
        )
...
def load_and_upsert_detector_prompt_with_session(session, filename: str) -> str:
    ...
    prompt_sha256 = hashlib.sha256(prompt_text.encode()).hexdigest()
    _upsert_prompt_with_session(session, prompt_sha256, prompt_text, None, None, filename)
    return prompt_sha256
