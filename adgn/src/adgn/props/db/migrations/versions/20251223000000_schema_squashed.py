...
# Issue ID format constraint (matches adgn.props.ids.BaseIssueID):
...
Raises exception if line numbers exceed file bounds or file not found in snapshot_files.'
    ...
    op.execute("COMMENT ON COLUMN agent_definitions.archive IS 'Uncompressed tar archive of the definition directory'")
    op.execute(
...
For failed critics: synthesizes zero-credit rows for catchable occurrences.'
    ...
    # critic_run_occurrence_stats view
    # NOTE: occurrences_caught_stats contains stats of RAW total_credit per grader run.
    # Consumer should compute recall ratio as: stats.mean / n_catchable_occurrences
    op.execute("""
        ...
        SELECT
            ...
            (SELECT COUNT(*)::integer FROM per_example pe
             WHERE pe.critic_definition_id = cros.critic_definition_id
               ...
               AND pe.example_kind = cros.example_kind) AS n_examples,
...
- occurrences_caught_stats: Stats across all critic runs; failed runs count as 0 credit (.n = total run count)'
    ...
    # aggregated_recall_by_example view
    op.execute("""
    ...
    op.execute("""
        ...
        WITH credit_stats_per_definition_example AS (
            SELECT
                ...
                cros.critic_definition_id,
                cros.critic_model,
                compute_stats_with_ci(array_agg(COALESCE((cros.occurrences_caught_stats).mean, 0.0))) AS credit_stats
            FROM critic_run_occurrence_stats cros
            GROUP BY cros.snapshot_slug, cros.split, cros.example_kind, cros.files_hash,
                cros.critic_definition_id, cros.critic_model
        ), best_scores AS (
            SELECT
                snapshot_slug,
                split,
        ...
        JOIN credit_stats_per_definition_example cspde ON (
            ...
            bs.example_kind = cspde.example_kind AND
            COALESCE(bs.files_hash, '') = COALESCE(cspde.files_hash, '') AND
        ...
        GROUP BY bs.snapshot_slug, bs.split, bs.example_kind, bs.files_hash,
            bs.n_catchable_occurrences, bs.critic_model, bs.best_mean_credit
    """)

    op.execute("""
        COMMENT ON VIEW pareto_frontier_by_example IS
        'Pareto frontier: best total credit achieved on each example and which agent definitions achieved it.
...
Failed critic runs (max_turns/context_length) count as 0.0 credit.'
    ...
    op.execute("GRANT SELECT ON TABLE critic_run_occurrence_stats TO agent_base")
    op.execute("GRANT SELECT ON TABLE aggregated_recall_by_definition TO agent_base")
    op.execute("GRANT SELECT ON TABLE aggregated_recall_by_example TO agent_base")
    ...
    # Snapshots policies (clustering user patterns)
    op.execute(
        "CREATE POLICY clustering_user_snapshots_policy ON snapshots FOR SELECT USING (CURRENT_USER ~ '^clustering_run_[0-9]+_agent$')"
    )
    op.execute(
        "CREATE POLICY clustering_user_true_positives_policy ON true_positives FOR SELECT USING (CURRENT_USER ~ '^clustering_run_[0-9]+_agent$')"
    )
    op.execute(
        "CREATE POLICY clustering_user_false_positives_policy ON false_positives FOR SELECT USING (CURRENT_USER ~ '^clustering_run_[0-9]+_agent$')"
    )
    op.execute(
        "CREATE POLICY clustering_user_events_policy ON events FOR SELECT USING (CURRENT_USER ~ '^clustering_run_[0-9]+_agent$')"
    )
    ...
    # True positives - prompt optimizer sees TRAIN, grader sees graded snapshot, improvement sees allowed
    op.execute("""
    ...
    # False positives - prompt optimizer sees TRAIN, grader sees graded snapshot, improvement sees allowed
    op.execute("""
