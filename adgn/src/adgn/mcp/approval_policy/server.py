...
class ProposeInput(BaseModel):
    source: str = Field(
        description="Python source code for the approval policy proposal"
    )
    rationale: str | None = Field(
        default=None, description="Optional rationale for the proposal"
    )
...
class ApprovalPolicyServer(NotifyingFastMCP):
    ...
    async def propose_tool(
        self, source: str, rationale: str | None = None
    ) -> ProposalCreated:
