...
class ApprovalPolicyServer(NotifyingFastMCP):
    ...
    def __init__(
        ...
        *,
        ...
        self._session_subscriptions: defaultdict[ServerSession, set[AnyUrl]] = (
            defaultdict(set)
        )
    ...
    def _register_resources(self) -> None:
        ...
        async def proposal_item(id: str) -> str:
            if (
                got := await self._engine.persistence.get_policy_proposal(
                    self._engine.agent_id, id
                )
            ) is None:
                raise KeyError(id)
        ...
        async def decide(input: PolicyRequest) -> PolicyResponse:
            ...
            evaluator = ContainerPolicyEvaluator(
                agent_id=self._engine.agent_id,
                docker_client=self._engine.docker_client,
                engine=self._engine,
            )
