...
def test_editor_content_respects_commit_verbose_config(temp_repo, git_repo):
    ...
    content = render_editor_content(git_repo, "test msg", [], stats_line="stats")
    ...
    content2 = render_editor_content(git_repo, "test msg", [], stats_line="stats")
...
def test_editor_content_includes_user_context(temp_repo, git_repo):
    ...
    content = render_editor_content(git_repo, "test msg", [], user_context="fixing the auth bug", stats_line="stats")
...
def test_editor_content_includes_previous_message_when_amending(temp_repo, git_repo):
    ...
    content = render_editor_content(git_repo, "new msg", [], previous_message="old commit message", stats_line="stats")
...
def test_editor_content_includes_both_user_context_and_previous_message(temp_repo, git_repo):
    ...
    content = render_editor_content(
        git_repo, "new msg", [], previous_message="old commit", user_context="user hint", stats_line="stats"
    )
...
def test_staged_files_not_duplicated_in_unstaged_section(temp_repo, git_repo, snapshot):
    ...
    content = render_editor_content(git_repo, "test", [])
