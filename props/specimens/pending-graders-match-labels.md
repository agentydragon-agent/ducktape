...
### `cli-help-port-mismatch.yaml` / `occ-0`
...
```
       5:
       6: Usage:
       7:     adgn-mcp-bridge serve --agent-id external-chatgpt \\
       8:         --db-path ./bridge.db \\
       9:         --mcp-config ./docker-exec.json \\
>>>   10:         --host 127.0.0.1 --port 8080
      ...
      15: import asyncio
   ...
      59:     "--mcp-config", type=Path, help="Path to .mcp.json config (servers to mount, e.g., docker exec with repo mount)"
      60: )
      61: @click.option("--host", default="127.0.0.1", help="Bind host")
      62: @click.option("--mcp-port", type=int, default=8080, help="MCP server port (token-authenticated)")
      63: @click.option("--ui-port", type=int, default=8081, help="Management UI port (WebSocket channels, no token auth)")
>>>   64: @click.option("--initial-policy", type=Path, help="Path to initial approval policy (Python file)")
>>>   65: def serve(
      66:     agent_id: str | None,
      67:     auth_tokens: Path | None,
      68:     db_path: Path,
      69:     mcp_config: Path | None,
      70:     host: str,
```

### `created-at-auto-default.yaml` / `occ-0`
...
```
      27:     """
      28:
      29:     __tablename__ = "agents"
      30:
      31:     id: Mapped[str] = mapped_column(String, primary_key=True)
>>>   32:     created_at: Mapped[datetime] = mapped_column(DateTime, nullable=False)
      33:     mcp_config: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False)  # MCPConfig as JSON
      34:     preset: Mapped[str] = mapped_column(String, nullable=False)  # AgentMetadata.preset
      35:
      36:     # Relationships
      37:     runs: Mapped[list["Run"]] = relationship(
```

### `created-at-auto-default.yaml` / `occ-1`
...
```
     114:     run_id: Mapped[str | None] = mapped_column(ForeignKey("runs.id", ondelete="CASCADE"), nullable=True)
     115:     agent_id: Mapped[str] = mapped_column(ForeignKey("agents.id", ondelete="CASCADE"), nullable=False)
     116:     tool_call_json: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False)  # ToolCall as JSON
     117:     decision_json: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)  # Decision as JSON
     118:     execution_json: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)  # ToolCallExecution as JSON
>>>  119:     created_at: Mapped[datetime] = mapped_column(DateTime, nullable=False)
     120:     decided_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
     121:     completed_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
     122:
     123:     # Relationships
     124:     run: Mapped["Run | None"] = relationship(back_populates="tool_calls")
```

### `created-at-auto-default.yaml` / `occ-2`
...
>>>  153:     created_at: Mapped[datetime] = mapped_column(DateTime, nullable=False)
     ...
     155:
     156:     # Relationships
     157:     agent: Mapped["Agent"] = relationship(back_populates="policies")
     158:
```
...
### `extract-cmd-use-shlex-join.yaml` / `occ-0`

File: `adgn/src/adgn/agent/server/reducer.py`

> Inline logic parses args_json to extract cmd from argv, using hand-rolled shell
> quoting (reducer.py:69-88):
>
> cmd: str | None = None
> parsed*args: dict | None = None
> if evt.args_json:
> try:
...
> if isinstance(argv, list): # shell-join with conservative quoting
> parts: list[str] = []
> for a in argv:
> if isinstance(a, str) and a and all(ch.isalnum() or ch in "*./-" for ch in a):
> parts.append(a)
> else:
> s = str(a).replace("'", "'\\''")
> parts.append(f"'{s}'")
...
```
      64:     if isinstance(evt, UiEndTurnEvt):
      65:         return append_item(state, EndTurnItem())
      66:
      67:     # Tool call start → begin a group (attempt to derive cmd from args_json for exec tools)
      68:     if isinstance(evt, ToolCall):
>>>   69:         cmd: str | None = None
>>>   70:         parsed_args: dict | None = None
>>>   71:         if evt.args_json:
>>>   72:             try:
>>>   73:                 args = json.loads(evt.args_json)
>>>   74:                 parsed_args = args if isinstance(args, dict) else None
>>>   75:                 argv = args.get("argv") or args.get("cmd") if isinstance(args, dict) else None
>>>   76:                 if isinstance(argv, list):
>>>   77:                     # shell-join with conservative quoting
>>>   78:                     parts: list[str] = []
>>>   79:                     for a in argv:
>>>   80:                         if isinstance(a, str) and a and all(ch.isalnum() or ch in "_./-" for ch in a):
>>>   81:                             parts.append(a)
>>>   82:                         else:
>>>   83:                             s = str(a).replace("'", "'\\''")
>>>   84:                             parts.append(f"'{s}'")
...
>>>   87:                 cmd = None
>>>   88:                 parsed_args = None
      89:         # For ui.send_message and ui.end_turn: do not create a ToolItem; UiMessageEvt/UiEndTurnEvt after execution will surface AssistantMarkdown/EndTurn
      90:         if evt.name in (
      91:             build_mcp_function(UI_SERVER_NAME, WellKnownTools.SEND_MESSAGE),
      92:             build_mcp_function(UI_SERVER_NAME, WellKnownTools.END_TURN),
      93:         ):
```
...
### `test-fixture-duplication.yaml` / `occ-0`

File: `adgn/tests/props/test_paths.py`

> The test file uses specimen_relative_path_model and mock_specimen_context repeatedly across many tests (lines
> 56, 59,
> 63, 88, etc.). These fixtures should be DRY'd up as shared fixtures. Additionally, the pattern of creating a
> wrapper
> Model class with a single field (specimen_relative_path_model fixture at conftest.py:38-44) and parsing with
> context
> is
> repeated across tests, suggesting a factory fixture would reduce duplication.

```
      51:
      52: # SnapshotRelativePath validation tests
      53:
      54:
      55: @pytest.mark.parametrize("path_str", ["src/module.py", "README.md", "a/b/c/d/file.txt"])
>>>   56: def test_valid_relative_paths(specimen_relative_path_model, mock_specimen_context: SpecimenContext, path_str: str):
>>>   57:     """Valid relative paths should be accepted with context."""
>>>   58:     ctx = {"snapshots": mock_specimen_context}
>>>   59:     m = specimen_relative_path_model.model_validate({"path": path_str}, context=ctx)
>>>   60:     assert_that(str(m.path), equal_to(path_str))
>>>   61:
>>>   62:
>>>   63: def test_accepts_string_or_path_input(specimen_relative_path_model, mock_specimen_context: SpecimenContext):
>>>   64:     """SnapshotRelativePath should accept both string and Path inputs."""
>>>   65:     ctx = {"snapshots": mock_specimen_context}
>>>   66:
>>>   67:     # String input
>>>   68:     m1 = specimen_relative_path_model.model_validate({"path": "src/module.py"}, context=ctx)
>>>   69:     assert_that(m1.path, instance_of(Path))
>>>   70:
>>>   71:     # Path input
>>>   72:     m2 = specimen_relative_path_model.model_validate({"path": Path("src/module.py")}, context=ctx)
>>>   73:     assert_that(m2.path, instance_of(Path))
>>>   74:
>>>   75:     # Both should be equal
>>>   76:     assert_that(m1.path, equal_to(m2.path))
>>>   77:
>>>   78:
>>>   79: @pytest.mark.parametrize(
>>>   80:     ("path_str", "error_match"),
>>>   81:     [
>>>   82:         ("/src/module.py", "absolute"),
>>>   83:         ("../other/file.py", "parent references"),
>>>   84:         ("src/../lib/file.py", "parent references"),
>>>   85:         ("", "empty"),
>>>   86:     ],
>>>   87: )
>>>   88: def test_rejects_invalid_format(specimen_relative_path_model, path_str, error_match):
>>>   89:     """Format validation should reject invalid paths (no context needed)."""
>>>   90:     with pytest.raises(ValidationError, match=error_match):
>>>   91:         specimen_relative_path_model(path=path_str)
>>>   92:
>>>   93:
>>>   94: def test_validation_without_context_skips_existence_check(specimen_relative_path_model):
>>>   95:     """Without snapshots, validation skips existence check (allows standalone parsing).
>>>   96:
>>>   97:     This enables parsing critiques without loading full specimen context.
>>>   98:     Format validation (relative path, no ..) still applies.
>>>   99:     """
>>>  100:     # Should succeed - format validation passes, existence check skipped
>>>  101:     result = specimen_relative_path_model(path="nonexistent/file.py")
>>>  102:     assert str(result.path) == "nonexistent/file.py"
>>>  103:
>>>  104:
>>>  105: def test_validation_with_wrong_context_key_skips_existence_check(specimen_relative_path_model):
>>>  106:     """With context dict but no snapshots key, existence check is skipped."""
>>>  107:     # Should succeed - format validation passes, existence check skipped
>>>  108:     result = specimen_relative_path_model.model_validate({"path": "nonexistent.py"}, context={"other_key": "value"})
>>>  109:     assert str(result.path) == "nonexistent.py"
>>>  110:
>>>  111:
>>>  112: def test_validation_with_context_checks_existence(specimen_relative_path_model, mock_specimen_context: SpecimenContext):
>>>  113:     """With snapshots, paths must exist in known_files."""
>>>  114:     ctx = {"snapshots": mock_specimen_context}
>>>  115:
>>>  116:     # Existing file should pass
>>>  117:     m = specimen_relative_path_model.model_validate({"path": "src/module.py"}, context=ctx)
>>>  118:     assert_that(m.path, equal_to(Path("src/module.py")))
>>>  119:
>>>  120:     # Non-existent file should fail
>>>  121:     with pytest.raises(ValidationError, match="not found"):
>>>  122:         specimen_relative_path_model.model_validate({"path": "nonexistent.py"}, context=ctx)
>>>  123:
>>>  124:
>>>  125: def test_validation_rejects_symlinks(specimen_relative_path_model, tmp_path: Path):
>>>  126:     """With context, symlinks should be rejected as issue anchors."""
>>>  127:     # Create a symlink
>>>  128:     target = tmp_path / "target.py"
>>>  129:     target.write_text("# mock")
>>>  130:     link = tmp_path / "link.py"
>>>  131:     link.symlink_to(target)
>>>  132:
>>>  133:     ctx_obj = SpecimenContext.from_hydrated_specimen(
>>>  134:         snapshot_slug=SnapshotSlug("test/symlink"), hydrated_root=tmp_path, specimen_issues=[], specimen_fps=[]
>>>  135:     )
>>>  136:
>>>  137:     with pytest.raises(ValidationError, match="regular file"):
>>>  138:         specimen_relative_path_model.model_validate({"path": "link.py"}, context={"snapshots": ctx_obj})
>>>  139:
>>>  140:
>>>  141: def test_validation_rejects_directories(specimen_relative_path_model, tmp_path: Path):
>>>  142:     """With context, directories should be rejected as issue anchors."""
>>>  143:     (tmp_path / "src").mkdir()
>>>  144:
>>>  145:     ctx_obj = SpecimenContext.from_hydrated_specimen(
>>>  146:         snapshot_slug=SnapshotSlug("test/dir"), hydrated_root=tmp_path, specimen_issues=[], specimen_fps=[]
>>>  147:     )
>>>  148:
>>>  149:     with pytest.raises(ValidationError, match="regular file"):
>>>  150:         specimen_relative_path_model.model_validate({"path": "src"}, context={"snapshots": ctx_obj})
>>>  151:
>>>  152:
>>>  153: def test_validation_multiple_files_in_known_files(specimen_relative_path_model, mock_specimen_context: SpecimenContext):
>>>  154:     """Test validation with multiple files in known_files."""
>>>  155:     ctx = {"snapshots": mock_specimen_context}
>>>  156:
>>>  157:     # All regular files should pass
>>>  158:     specimen_relative_path_model.model_validate({"path": "src/module.py"}, context=ctx)
>>>  159:     specimen_relative_path_model.model_validate({"path": "README.md"}, context=ctx)
>>>  160:     specimen_relative_path_model.model_validate({"path": "single.txt"}, context=ctx)
>>>  161:
>>>  162:     # Unknown file should fail
>>>  163:     with pytest.raises(ValidationError, match="not found"):
>>>  164:         specimen_relative_path_model.model_validate({"path": "unknown.py"}, context=ctx)
>>>  165:
>>>  166:
>>>  167: # Serialization tests
>>>  168:
>>>  169:
>>>  170: def test_json_serialization(specimen_relative_path_model, mock_specimen_context: SpecimenContext):
>>>  171:     """SnapshotRelativePath should serialize to string in JSON mode."""
>>>  172:     ctx = {"snapshots": mock_specimen_context}
>>>  173:     m = specimen_relative_path_model.model_validate({"path": "src/module.py"}, context=ctx)
>>>  174:     json_data = m.model_dump(mode="json")
>>>  175:     assert_that(json_data["path"], equal_to("src/module.py"))
>>>  176:
>>>  177:
>>>  178: def test_python_serialization_returns_path(specimen_relative_path_model, mock_specimen_context: SpecimenContext):
>>>  179:     """SnapshotRelativePath should return Path in Python mode."""
>>>  180:     ctx = {"snapshots": mock_specimen_context}
>>>  181:     m = specimen_relative_path_model.model_validate({"path": "src/module.py"}, context=ctx)
>>>  182:     python_data = m.model_dump(mode="python")
>>>  183:     assert_that(python_data["path"], equal_to(Path("src/module.py")))
>>>  184:
>>>  185:
>>>  186: def test_round_trip(specimen_relative_path_model, mock_specimen_context: SpecimenContext):
>>>  187:     """SnapshotRelativePath should round-trip through JSON."""
>>>  188:     ctx = {"snapshots": mock_specimen_context}
>>>  189:     m1 = specimen_relative_path_model.model_validate({"path": "src/module.py"}, context=ctx)
>>>  190:     json_str = m1.model_dump_json()
>>>  191:     m2 = specimen_relative_path_model.model_validate_json(json_str, context=ctx)
>>>  192:     assert_that(m1.path, equal_to(m2.path))
>>>  193:
>>>  194:
>>>  195: def test_round_trip_via_dict(specimen_relative_path_model, mock_specimen_context: SpecimenContext):
>>>  196:     """SnapshotRelativePath can round-trip through dict (workaround for JSON issue)."""
>>>  197:     ctx = {"snapshots": mock_specimen_context}
>>>  198:     m1 = specimen_relative_path_model.model_validate({"path": "src/module.py"}, context=ctx)
>>>  199:     json_dict = m1.model_dump(mode="json")
>>>  200:     m2 = specimen_relative_path_model.model_validate(json_dict, context=ctx)
>>>  201:     assert_that(m1.path, equal_to(m2.path))
>>>  202:     assert_that(m2.path, equal_to(Path("src/module.py")))
>>>  203:
>>>  204:
>>>  205: # Edge cases
>>>  206:
>>>  207:
>>>  208: def test_single_component_path(specimen_relative_path_model, mock_specimen_context: SpecimenContext):
>>>  209:     """Single component paths should work (e.g., README.md)."""
>>>  210:     ctx = {"snapshots": mock_specimen_context}
>>>  211:     m = specimen_relative_path_model.model_validate({"path": "README.md"}, context=ctx)
>>>  212:     assert_that(m.path, equal_to(Path("README.md")))
>>>  213:
>>>  214:
>>>  215: def test_path_with_dots_in_filename(specimen_relative_path_model, mock_specimen_context: SpecimenContext):
>>>  216:     """Paths with dots in filename (not ..) should work."""
>>>  217:     ctx = {"snapshots": mock_specimen_context}
>>>  218:     m = specimen_relative_path_model.model_validate({"path": "src/file.test.py"}, context=ctx)
>>>  219:     assert_that(m.path, equal_to(Path("src/file.test.py")))
>>>  220:
>>>  221:
>>>  222: def test_path_normalization(specimen_relative_path_model, mock_specimen_context: SpecimenContext):
>>>  223:     """Path should be normalized (extra slashes removed)."""
>>>  224:     ctx = {"snapshots": mock_specimen_context}
>>>  225:     # Path constructor normalizes this
>>>  226:     m = specimen_relative_path_model.model_validate({"path": "src//module.py"}, context=ctx)
>>>  227:     # Path normalizes to src/module.py
>>>  228:     assert_that(m.path.parts, contains_inanyorder("src", "module.py"))
```
...
File: `adgn/tests/mcp/approval_policy/test_proposals_resources.py`
...
```
      ...
      22: async def test_create_and_list_proposals(make_typed_mcp, approval_engine):
      23:     """Test creating proposals and listing them via resources."""
      24:     # Mount proposer server for creating proposals
>>>   25:     proposer = ApprovalPolicyProposerServer(engine=approval_engine)
      26:
      27:     async with make_typed_mcp(proposer, "approval_policy.proposer") as (proposer_client, _sess):
      28:         # Create a proposal
      29:         result = await proposer_client.call_tool(
      30:             "create_proposal",
   ...
      27:     async with make_typed_mcp(proposer, "approval_policy.proposer") as (proposer_client, _sess):
      28:         # Create a proposal
      29:         result = await proposer_client.call_tool(
      30:             "create_proposal",
      31:             CreateProposalArgs(
>>>   32:                 content="""
>>>   33: class ApprovalPolicy:
>>>   34:     def decide(self, name: str, arguments: dict) -> dict:
>>>   35:         return {"decision": "allow"}
>>>   36: """
      37:             ),
      38:         )
      39:         proposal = ProposalDescriptor.model_validate(result.structured_content)
      40:         assert_that(proposal.status, equal_to(ProposalStatus.PENDING))
      41:         assert proposal.id
   ...
      39:         proposal = ProposalDescriptor.model_validate(result.structured_content)
      40:         assert_that(proposal.status, equal_to(ProposalStatus.PENDING))
      41:         assert proposal.id
      42:
      43:     # Now mount readonly server to access resources
>>>   44:     reader = ApprovalPolicyServer(approval_engine)
      45:     async with make_typed_mcp(reader, "approval_policy") as (reader_client, sess):
      46:         # List proposals via resource
      47:         proposals = await read_text_json_typed(
      48:             sess, f"{APPROVAL_POLICY_PROPOSALS_INDEX_URI}/list", list[ProposalDescriptor]
      49:         )
   ...
      54:
      55: @pytest.mark.requires_docker
      56: async def test_get_proposal_detail(make_typed_mcp, approval_engine):
      57:     """Test retrieving full proposal details via resource."""
      58:     # Create a proposal first
>>>   59:     proposer = ApprovalPolicyProposerServer(engine=approval_engine)
      60:     test_content = """
      61: class ApprovalPolicy:
      62:     def decide(self, name: str, arguments: dict) -> dict:
      63:         return {"decision": "allow"}
      64: """
   ...
      55: @pytest.mark.requires_docker
      56: async def test_get_proposal_detail(make_typed_mcp, approval_engine):
...
>>>   64: """
      65:
      66:     async with make_typed_mcp(proposer, "approval_policy.proposer") as (proposer_client, _sess):
      67:         result = await proposer_client.call_tool("create_proposal", CreateProposalArgs(content=test_content))
      68:         proposal = ProposalDescriptor.model_validate(result.structured_content)
      69:
   ...
      66:     async with make_typed_mcp(proposer, "approval_policy.proposer") as (proposer_client, _sess):
      67:         result = await proposer_client.call_tool("create_proposal", CreateProposalArgs(content=test_content))
      68:         proposal = ProposalDescriptor.model_validate(result.structured_content)
      69:
      70:     # Read proposal detail via resource
>>>   71:     reader = ApprovalPolicyServer(approval_engine)
      72:     async with make_typed_mcp(reader, "approval_policy") as (reader_client, sess):
      73:         detail = await read_text_json_typed(
      74:             sess, f"{APPROVAL_POLICY_PROPOSALS_INDEX_URI}/{proposal.id}", ProposalDetail
      75:         )
      76:         assert_that(detail.id, equal_to(proposal.id))
   ...
      81:
      82:
      83: @pytest.mark.requires_docker
      84: async def test_approve_proposal_workflow(make_typed_mcp, approval_engine):
      85:     """Test complete approval workflow: create, list, approve, verify."""
>>>   86:     test_content = """
>>>   87: class ApprovalPolicy:
>>>   88:     def decide(self, name: str, arguments: dict) -> dict:
>>>   89:         return {"decision": "allow"}
>>>   90: """
      91:
      92:     # Create proposal
      93:     proposer = ApprovalPolicyProposerServer(engine=approval_engine)
      94:     async with make_typed_mcp(proposer, "approval_policy.proposer") as (proposer_client, _sess):
      95:         result = await proposer_client.call_tool("create_proposal", CreateProposalArgs(content=test_content))
   ...
      88:     def decide(self, name: str, arguments: dict) -> dict:
      89:         return {"decision": "allow"}
      90: """
...
>>>   93:     proposer = ApprovalPolicyProposerServer(engine=approval_engine)
      94:     async with make_typed_mcp(proposer, "approval_policy.proposer") as (proposer_client, _sess):
      95:         result = await proposer_client.call_tool("create_proposal", CreateProposalArgs(content=test_content))
      96:         proposal = ProposalDescriptor.model_validate(result.structured_content)
      97:
      98:     # Approve proposal
   ...
      94:     async with make_typed_mcp(proposer, "approval_policy.proposer") as (proposer_client, _sess):
      ...
      97:
      98:     # Approve proposal
>>>   99:     admin = ApprovalPolicyAdminServer(engine=approval_engine)
     100:     async with make_typed_mcp(admin, "approval_policy.approver") as (admin_client, _sess):
     101:         await admin_client.call_tool(
     102:             "approve_proposal", ApproveProposalArgs(id=proposal.id, comment="Looks good to me")
     103:         )
     104:
   ...
     101:         await admin_client.call_tool(
     ...
     103:         )
     104:
     105:     # Verify proposal is now approved
>>>  106:     reader = ApprovalPolicyServer(approval_engine)
     107:     async with make_typed_mcp(reader, "approval_policy") as (reader_client, sess):
     108:         detail = await read_text_json_typed(
     109:             sess, f"{APPROVAL_POLICY_PROPOSALS_INDEX_URI}/{proposal.id}", ProposalDetail
     110:         )
     111:         assert_that(detail.status, equal_to(ProposalStatus.APPROVED))
   ...
     122:
     123:
     124: @pytest.mark.requires_docker
     125: async def test_reject_proposal_workflow(make_typed_mcp, approval_engine):
     126:     """Test complete rejection workflow: create, list, reject, verify."""
>>>  127:     test_content = """
>>>  128: class ApprovalPolicy:
>>>  129:     def decide(self, name: str, arguments: dict) -> dict:
>>>  130:         return {"decision": "deny_abort"}
>>>  131: """
     132:
     133:     # Create proposal
     134:     proposer = ApprovalPolicyProposerServer(engine=approval_engine)
     135:     async with make_typed_mcp(proposer, "approval_policy.proposer") as (proposer_client, _sess):
     136:         result = await proposer_client.call_tool("create_proposal", CreateProposalArgs(content=test_content))
   ...
     129:     def decide(self, name: str, arguments: dict) -> dict:
     130:         return {"decision": "deny_abort"}
     131: """
...
>>>  134:     proposer = ApprovalPolicyProposerServer(engine=approval_engine)
     135:     async with make_typed_mcp(proposer, "approval_policy.proposer") as (proposer_client, _sess):
     136:         result = await proposer_client.call_tool("create_proposal", CreateProposalArgs(content=test_content))
     137:         proposal = ProposalDescriptor.model_validate(result.structured_content)
     138:
     139:     # Reject proposal
   ...
     135:     async with make_typed_mcp(proposer, "approval_policy.proposer") as (proposer_client, _sess):
     ...
     138:
     139:     # Reject proposal
>>>  140:     admin = ApprovalPolicyAdminServer(engine=approval_engine)
     141:     async with make_typed_mcp(admin, "approval_policy.approver") as (admin_client, _sess):
     142:         await admin_client.call_tool(
     143:             "reject_proposal", RejectProposalArgs(id=proposal.id, reason="Too restrictive")
     144:         )
     145:
   ...
     142:         await admin_client.call_tool(
     ...
     144:         )
     145:
     146:     # Verify proposal is now rejected
>>>  147:     reader = ApprovalPolicyServer(approval_engine)
     148:     async with make_typed_mcp(reader, "approval_policy") as (reader_client, sess):
     149:         detail = await read_text_json_typed(
     150:             sess, f"{APPROVAL_POLICY_PROPOSALS_INDEX_URI}/{proposal.id}", ProposalDetail
     151:         )
     152:         assert_that(detail.status, equal_to(ProposalStatus.REJECTED))
   ...
     154:
     155:
     156: @pytest.mark.requires_docker
     157: async def test_list_proposals_with_multiple_statuses(make_typed_mcp, approval_engine):
     158:     """Test listing proposals with different statuses (pending, approved, rejected)."""
>>>  159:     test_content_allow = """
>>>  160: class ApprovalPolicy:
>>>  161:     def decide(self, name: str, arguments: dict) -> dict:
>>>  162:         return {"decision": "allow"}
>>>  163: """
>>>  164:     test_content_deny = """
>>>  165: class ApprovalPolicy:
>>>  166:     def decide(self, name: str, arguments: dict) -> dict:
>>>  167:         return {"decision": "deny_continue"}
>>>  168: """
     169:
     170:     # Create multiple proposals
     171:     proposer = ApprovalPolicyProposerServer(engine=approval_engine)
     172:     proposal_ids = []
     173:
   ...
     166:     def decide(self, name: str, arguments: dict) -> dict:
...
>>>  171:     proposer = ApprovalPolicyProposerServer(engine=approval_engine)
     172:     proposal_ids = []
     173:
     174:     async with make_typed_mcp(proposer, "approval_policy.proposer") as (proposer_client, _sess):
     175:         # Create 3 proposals
     176:         for content in [test_content_allow, test_content_deny, test_content_allow]:
   ...
     176:         for content in [test_content_allow, test_content_deny, test_content_allow]:
     177:             proposal = await proposer_client.create_proposal(CreateProposalArgs(content=content))
     178:             proposal_ids.append(proposal.id)
     179:
     180:     # Approve first, reject second, leave third pending
>>>  181:     admin = ApprovalPolicyAdminServer(engine=approval_engine)
     182:     async with make_typed_mcp(admin, "approval_policy.approver") as (admin_client, _sess):
     183:         await admin_client.approve_proposal(ApproveProposalArgs(id=proposal_ids[0]))
     184:         await admin_client.reject_proposal(RejectProposalArgs(id=proposal_ids[1]))
     185:
     186:     # Verify all proposals appear in list with correct statuses
   ...
     182:     async with make_typed_mcp(admin, "approval_policy.approver") as (admin_client, _sess):
     183:         await admin_client.approve_proposal(ApproveProposalArgs(id=proposal_ids[0]))
     184:         await admin_client.reject_proposal(RejectProposalArgs(id=proposal_ids[1]))
     185:
     186:     # Verify all proposals appear in list with correct statuses
>>>  187:     reader = ApprovalPolicyServer(approval_engine)
     188:     async with make_typed_mcp(reader, "approval_policy") as (reader_client, sess):
     189:         proposals = await read_text_json_typed(
     190:             sess, f"{APPROVAL_POLICY_PROPOSALS_INDEX_URI}/list", list[ProposalDescriptor]
     191:         )
     192:         assert_that(proposals, has_length(3))
   ...
     199:
     200:
     201: @pytest.mark.requires_docker
     202: async def test_proposal_detail_not_found(make_typed_mcp, approval_engine):
     203:     """Test that accessing non-existent proposal raises KeyError."""
>>>  204:     reader = ApprovalPolicyServer(approval_engine)
     205:     async with make_typed_mcp(reader, "approval_policy") as (reader_client, sess):
     206:         # Try to read a non-existent proposal
     207:         with pytest.raises(Exception) as exc_info:
     208:             await reader_client.read_resource(f"{APPROVAL_POLICY_PROPOSALS_INDEX_URI}/nonexistent-id")
     209:         # The error should propagate (exact message may vary)
```
...
### `approvals-history-count.yaml` / `occ-0`
...
```
     163:
     164: class AgentApprovalsHistory(BaseModel):
     165:     """Content for resource://agents/{id}/approvals/history."""
     166:
     167:     agent_id: AgentID
>>>  168:     timeline: list[ApprovalHistoryEntry]
     169:     pending: list[PendingApproval]  # Pending approvals not yet decided
     170:     count: int  # Total count (timeline + pending)
     171:
     172:
     173: class PolicyProposalInfo(BaseModel):
   ...
     441:         for record in agent_records:
     442:             entry = _convert_tool_call_record_to_history(record)
     443:             if entry is not None:
     444:                 completed_entries.append(entry)
     445:
>>>  446:         pending_approvals = _convert_pending_approvals(infra.approval_hub.pending)
>>>  447:
>>>  448:         # Count includes both completed and pending
>>>  449:         total_count = len(completed_entries) + len(pending_approvals)
>>>  450:
     451:         return AgentApprovalsHistory(
     452:             agent_id=agent_id, timeline=completed_entries, pending=pending_approvals, count=total_count
     453:         )
     454:
     455:     @server.resource(
   ...
...
### `multiple-toolcallrecord.yaml` / `occ-0`
...
```
     137:         self._policy_reader = policy_reader
     138:         self._persistence = persistence
     139:         self._run_id = run_id
     140:         self._agent_id = agent_id
     141:
>>>  142:     async def on_call_tool(self, context: MiddlewareContext[Any], call_next: CallNext[Any, ToolResult]) -> ToolResult:
>>>  143:         name = context.message.name
>>>  144:         arguments = context.message.arguments
>>>  145:         tool_key = name  # canonical function name
>>>  146:         call_id = "pg:" + uuid.uuid4().hex
>>>  147:
>>>  148:         # Create PENDING tool call record
>>>  149:         tool_call = ToolCall(name=name, call_id=call_id, args_json=json.dumps(arguments) if arguments else None)
>>>  150:         pending_record = ToolCallRecord(
>>>  151:             call_id=call_id,
>>>  152:             run_id=str(self._run_id) if self._run_id is not None else None,
>>>  153:             agent_id=self._agent_id,
>>>  154:             tool_call=tool_call,
>>>  155:             decision=None,
>>>  156:             execution=None,
>>>  157:         )
>>>  158:         await self._persistence.save_tool_call(pending_record)
>>>  159:
>>>  160:         # Evaluate decision via MCP reader server when available; fallback to local evaluator
>>>  161:         try:
>>>  162:             decision_res = await self._policy_reader.decide(PolicyRequest(name=name, arguments=arguments))
>>>  163:             decision = decision_res.decision
>>>  164:             rationale = decision_res.rationale
>>>  165:         except Exception as e:  # policy engine failure → explicit evaluator error
>>>  166:             logger.warning("policy evaluator error", exc_info=e)
>>>  167:             raise McpError(
>>>  168:                 ErrorData(
>>>  169:                     code=POLICY_EVALUATOR_ERROR_CODE,
>>>  170:                     message=POLICY_EVALUATOR_ERROR_MSG,
>>>  171:                     data={POLICY_GATEWAY_STAMP_KEY: True, "name": name, "reason": f"{type(e).__name__}: {e}"},
>>>  172:                 )
>>>  173:             )
>>>  174:
>>>  175:         logger.debug("Policy decision: %s → %s (%s)", name, decision, rationale or "")
>>>  176:
>>>  177:         if decision is ApprovalDecision.ALLOW:
>>>  178:             # Update with decision (PENDING → EXECUTING)
>>>  179:             decision_obj = Decision(outcome=ApprovalOutcome.POLICY_ALLOW, decided_at=_now(), reason=rationale)
>>>  180:             executing_record = ToolCallRecord(
>>>  181:                 call_id=call_id,
>>>  182:                 run_id=str(self._run_id) if self._run_id is not None else None,
>>>  183:                 agent_id=self._agent_id,
>>>  184:                 tool_call=ToolCall(name=name, call_id=call_id, args_json=json.dumps(arguments) if arguments else None),
>>>  185:                 decision=decision_obj,
>>>  186:                 execution=None,
>>>  187:             )
>>>  188:             await self._persistence.save_tool_call(executing_record)
>>>  189:
>>>  190:             try:
>>>  191:                 call_result = await call_next(context)
>>>  192:
>>>  193:                 # Update with execution result (EXECUTING → COMPLETED)
>>>  194:                 execution_obj = ToolCallExecution(completed_at=_now(), output=convert_fastmcp_result(call_result))
>>>  195:                 completed_record = ToolCallRecord(
>>>  196:                     call_id=call_id,
>>>  197:                     run_id=str(self._run_id) if self._run_id is not None else None,
>>>  198:                     agent_id=self._agent_id,
>>>  199:                     tool_call=ToolCall(
>>>  200:                         name=name, call_id=call_id, args_json=json.dumps(arguments) if arguments else None
>>>  201:                     ),
>>>  202:                     decision=decision_obj,
>>>  203:                     execution=execution_obj,
>>>  204:                 )
>>>  205:                 await self._persistence.save_tool_call(completed_record)
>>>  206:
>>>  207:                 # If downstream returned an error ToolResult instead of raising,
>>>  208:                 # remap reserved policy codes/messages here using typed parsing when available.
>>>  209:                 if bool(call_result.is_error):
>>>  210:                     # Parse error details - ErrorData guarantees code: int per MCP/JSON-RPC spec
>>>  211:                     err = call_result.error
>>>  212:                     if err is None:
>>>  213:                         return call_result
>>>  214:
>>>  215:                     # Try parsing as ErrorData (validates code is int, message is str)
>>>  216:                     try:
>>>  217:                         ed = mtypes.ErrorData.model_validate(err)
>>>  218:                     except Exception:
>>>  219:                         # Non-conforming error format - pass through
>>>  220:                         return call_result
>>>  221:
>>>  222:                     # Check if error uses reserved policy codes/messages
>>>  223:                     stamped_downstream = isinstance(ed.data, dict) and ed.data.get(POLICY_GATEWAY_STAMP_KEY) is True
>>>  224:                     if (
>>>  225:                         stamped_downstream
>>>  226:                         or ed.code
>>>  227:                         in (POLICY_DENIED_ABORT_CODE, POLICY_DENIED_CONTINUE_CODE, POLICY_EVALUATOR_ERROR_CODE)
>>>  228:                         or ed.message
>>>  229:                         in (POLICY_DENIED_ABORT_MSG, POLICY_DENIED_CONTINUE_MSG, POLICY_EVALUATOR_ERROR_MSG)
>>>  230:                     ):
>>>  231:                         raise McpError(
>>>  232:                             ErrorData(
>>>  233:                                 code=POLICY_BACKEND_RESERVED_MISUSE_CODE,
>>>  234:                                 message=POLICY_BACKEND_RESERVED_MISUSE_MSG,
>>>  235:                                 data={POLICY_GATEWAY_STAMP_KEY: True, "name": name, "backend_code": ed.code},
>>>  236:                             )
>>>  237:                         )
>>>  238:                 return call_result
>>>  239:             except McpError as e:
>>>  240:                 _raise_if_reserved_code(e, name)
>>>  241:                 raise
>>>  242:             except Exception as e:
>>>  243:                 # Some servers may translate backend McpError into a ToolError before it reaches us.
>>>  244:                 # As a last resort, remap by inspecting the exception text.
>>>  245:                 s = str(e)
>>>  246:                 if (
>>>  247:                     (POLICY_DENIED_ABORT_MSG in s)
>>>  248:                     or (POLICY_DENIED_CONTINUE_MSG in s)
>>>  249:                     or (POLICY_EVALUATOR_ERROR_MSG in s)
>>>  250:                 ):
>>>  251:                     raise McpError(
>>>  252:                         ErrorData(
>>>  253:                             code=POLICY_BACKEND_RESERVED_MISUSE_CODE,
>>>  254:                             message=POLICY_BACKEND_RESERVED_MISUSE_MSG,
>>>  255:                             data={POLICY_GATEWAY_STAMP_KEY: True, "name": name, "backend_code": "unknown"},
>>>  256:                         )
>>>  257:                     )
>>>  258:                 raise
>>>  259:
>>>  260:         if decision is ApprovalDecision.DENY_ABORT:
>>>  261:             # Update with decision (no execution)
>>>  262:             decision_obj = Decision(outcome=ApprovalOutcome.POLICY_DENY_ABORT, decided_at=_now(), reason=rationale)
>>>  263:             denied_record = ToolCallRecord(
>>>  264:                 call_id=call_id,
>>>  265:                 run_id=str(self._run_id) if self._run_id is not None else None,
>>>  266:                 agent_id=self._agent_id,
>>>  267:                 tool_call=ToolCall(name=name, call_id=call_id, args_json=json.dumps(arguments) if arguments else None),
>>>  268:                 decision=decision_obj,
>>>  269:                 execution=None,
>>>  270:             )
>>>  271:             await self._persistence.save_tool_call(denied_record)
>>>  272:
>>>  273:             raise _policy_denied_error(ApprovalDecision.DENY_ABORT, name, rationale)
>>>  274:
>>>  275:         if decision is ApprovalDecision.DENY_CONTINUE:
>>>  276:             # Update with decision (no execution)
>>>  277:             decision_obj = Decision(outcome=ApprovalOutcome.POLICY_DENY_CONTINUE, decided_at=_now(), reason=rationale)
>>>  278:             denied_record = ToolCallRecord(
>>>  279:                 call_id=call_id,
>>>  280:                 run_id=str(self._run_id) if self._run_id is not None else None,
>>>  281:                 agent_id=self._agent_id,
>>>  282:                 tool_call=ToolCall(name=name, call_id=call_id, args_json=json.dumps(arguments) if arguments else None),
>>>  283:                 decision=decision_obj,
>>>  284:                 execution=None,
>>>  285:             )
>>>  286:             await self._persistence.save_tool_call(denied_record)
>>>  287:
>>>  288:             raise _policy_denied_error(ApprovalDecision.DENY_CONTINUE, name, rationale)
>>>  289:
>>>  290:         # ASK: block until resolved via ApprovalHub
>>>  291:         req = ApprovalRequest(
>>>  292:             tool_call=ToolCall(name=name, call_id=call_id, args_json=(json.dumps(arguments) if arguments else None))
>>>  293:         )
>>>  294:         # Register + notify before awaiting
>>>  295:         wait_coro = self._hub.await_decision(call_id, req)
>>>  296:         if self._notify is not None:
>>>  297:             await self._notify(req.tool_call)
>>>  298:
>>>  299:         decision_response = await wait_coro
>>>  300:
>>>  301:         if isinstance(decision_response, ContinueDecision):
>>>  302:             # User approved - update with decision (PENDING → EXECUTING)
>>>  303:             decision_obj = Decision(outcome=ApprovalOutcome.USER_APPROVE, decided_at=_now(), reason=None)
>>>  304:             executing_record = ToolCallRecord(
>>>  305:                 call_id=call_id,
>>>  306:                 run_id=str(self._run_id) if self._run_id is not None else None,
>>>  307:                 agent_id=self._agent_id,
>>>  308:                 tool_call=ToolCall(name=name, call_id=call_id, args_json=json.dumps(arguments) if arguments else None),
>>>  309:                 decision=decision_obj,
>>>  310:                 execution=None,
>>>  311:             )
>>>  312:             await self._persistence.save_tool_call(executing_record)
>>>  313:
>>>  314:             try:
>>>  315:                 call_result = await call_next(context)
>>>  316:
>>>  317:                 # Update with execution result (EXECUTING → COMPLETED)
>>>  318:                 execution_obj = ToolCallExecution(completed_at=_now(), output=convert_fastmcp_result(call_result))
>>>  319:                 completed_record = ToolCallRecord(
>>>  320:                     call_id=call_id,
>>>  321:                     run_id=str(self._run_id) if self._run_id is not None else None,
>>>  322:                     agent_id=self._agent_id,
>>>  323:                     tool_call=ToolCall(
>>>  324:                         name=name, call_id=call_id, args_json=json.dumps(arguments) if arguments else None
>>>  325:                     ),
>>>  326:                     decision=decision_obj,
>>>  327:                     execution=execution_obj,
>>>  328:                 )
>>>  329:                 await self._persistence.save_tool_call(completed_record)
>>>  330:
>>>  331:                 return call_result
>>>  332:             except McpError as e:
>>>  333:                 _raise_if_reserved_code(e, name)
>>>  334:                 raise
>>>  335:
>>>  336:         if isinstance(decision_response, AbortTurnDecision):
>>>  337:             # User denied - update with decision (no execution)
>>>  338:             decision_obj = Decision(
>>>  339:                 outcome=ApprovalOutcome.USER_DENY_ABORT, decided_at=_now(), reason=decision_response.reason
>>>  340:             )
>>>  341:             denied_record = ToolCallRecord(
>>>  342:                 call_id=call_id,
>>>  343:                 run_id=str(self._run_id) if self._run_id is not None else None,
>>>  344:                 agent_id=self._agent_id,
>>>  345:                 tool_call=ToolCall(name=name, call_id=call_id, args_json=json.dumps(arguments) if arguments else None),
>>>  346:                 decision=decision_obj,
>>>  347:                 execution=None,
>>>  348:             )
>>>  349:             await self._persistence.save_tool_call(denied_record)
>>>  350:
>>>  351:             raise _policy_denied_error(ApprovalDecision.DENY_ABORT, name, decision_response.reason)
>>>  352:
>>>  353:         # Unknown decision type: internal error for visibility
>>>  354:         raise McpError(
>>>  355:             ErrorData(
>>>  356:                 code=-32603,
>>>  357:                 message="internal_error: unknown approval decision type",
>>>  358:                 data={POLICY_GATEWAY_STAMP_KEY: True, "name": name, "decision_type": type(decision_response).__name__},
     359:             )
     360:         )
     361:
     362:
     363: def install_policy_gateway(
   ...
     145:         tool_key = name  # canonical function name
     146:         call_id = "pg:" + uuid.uuid4().hex
     147:
...
### `notifier-wiring-duplication.yaml` / `occ-0`
...
```
     836:     # For approval changes: wire ApprovalHub notifier to broadcast MCP resource updates
     837:     # For policy changes: wire policy engine notifier to broadcast MCP resource updates
     838:
     839:     for agent_id in registry.known_agents():
     840:         infra = await registry.get_infrastructure(agent_id)
>>>  841:
>>>  842:         # Closure captures agent_id for this specific agent
>>>  843:         def make_policy_notifier(aid: str):
>>>  844:             def notifier(uri: str):
>>>  845:                 # Notifier is sync, schedule broadcast in event loop
>>>  846:                 loop = asyncio.get_running_loop()
>>>  847:                 _task = loop.create_task(server.broadcast_resource_updated(uri))
>>>  848:                 # Don't await task - fire and forget notification
>>>  849:                 _task.add_done_callback(
>>>  850:                     lambda t: logger.debug(f"Broadcast complete for {uri}")
>>>  851:                     if not t.exception()
>>>  852:                     else logger.warning(f"Broadcast failed for {uri}: {t.exception()}")
>>>  853:                 )
>>>  854:
>>>  855:             return notifier
     856:
     857:         infra.approval_engine.set_notifier(make_policy_notifier(agent_id))
     858:
     859:         # Wire approval hub notifier to broadcast resource updates
     860:         def make_approval_hub_notifier(aid: AgentID):
   ...
     853:                 )
     854:
     855:             return notifier
...
>>>  860:         def make_approval_hub_notifier(aid: AgentID):
>>>  861:             def notifier():
>>>  862:                 # Notifier is sync, schedule broadcast in event loop
>>>  863:                 loop = asyncio.get_running_loop()
>>>  864:                 # Broadcast all relevant approval resources
>>>  865:                 uris = [
>>>  866:                     resources.agent_approvals_pending(aid),
>>>  867:                     resources.agent_approvals_history(aid),
>>>  868:                     resources.APPROVALS_PENDING_GLOBAL,
>>>  869:                 ]
>>>  870:                 for uri in uris:
>>>  871:                     _task = loop.create_task(server.broadcast_resource_updated(uri))
>>>  872:                     _task.add_done_callback(
>>>  873:                         lambda t, u=uri: logger.debug(f"Broadcast complete for {u}")
>>>  874:                         if not t.exception()
>>>  875:                         else logger.warning(f"Broadcast failed for {u}: {t.exception()}")
>>>  876:                     )
>>>  877:
>>>  878:             return notifier
     879:
     880:         infra.approval_hub.set_notifier(make_approval_hub_notifier(agent_id))
     881:
     882:         # Wire UI state notifications (only for local agents with session)
     883:         local_runtime = registry.get_local_runtime(agent_id)
   ...
     879:
     880:         infra.approval_hub.set_notifier(make_approval_hub_notifier(agent_id))
     881:
     882:         # Wire UI state notifications (only for local agents with session)
     883:         local_runtime = registry.get_local_runtime(agent_id)
>>>  884:         if local_runtime is not None and local_runtime.session is not None:
>>>  885:
>>>  886:             def make_ui_state_notifier(aid: AgentID):
>>>  887:                 def notifier():
>>>  888:                     # Notifier is sync, schedule broadcast in event loop
>>>  889:                     loop = asyncio.get_running_loop()
>>>  890:                     _task = loop.create_task(server.broadcast_resource_updated(resources.agent_ui_state(aid)))
>>>  891:                     # Don't await task - fire and forget notification
>>>  892:                     _task.add_done_callback(
>>>  893:                         lambda t: logger.debug(f"UI state broadcast complete for {aid}")
>>>  894:                         if not t.exception()
>>>  895:                         else logger.warning(f"UI state broadcast failed for {aid}: {t.exception()}")
>>>  896:                     )
>>>  897:
>>>  898:                 return notifier
     899:
     900:             local_runtime.session.set_ui_state_notifier(make_ui_state_notifier(agent_id))
     901:
     902:             # Wire session state notifications
     903:             def make_session_state_notifier(aid: AgentID):
   ...
     896:                     )
     897:
     898:                 return notifier
...
>>>  903:             def make_session_state_notifier(aid: AgentID):
>>>  904:                 def notifier():
>>>  905:                     # Notifier is sync, schedule broadcast in event loop
>>>  906:                     loop = asyncio.get_running_loop()
>>>  907:                     _task = loop.create_task(server.broadcast_resource_updated(resources.agent_session_state(aid)))
>>>  908:                     # Don't await task - fire and forget notification
>>>  909:                     _task.add_done_callback(
>>>  910:                         lambda t: logger.debug(f"Session state broadcast complete for {aid}")
>>>  911:                         if not t.exception()
>>>  912:                         else logger.warning(f"Session state broadcast failed for {aid}: {t.exception()}")
>>>  913:                     )
>>>  914:
>>>  915:                 return notifier
     916:
     917:             local_runtime.session._manager.set_session_state_notifier(make_session_state_notifier(agent_id))
     918:
     919:         # Wire compositor mount events to broadcast MCP state resource updates
     920:         def make_mount_listener(aid: AgentID):
```

### `parallel-dicts-to-dataclass.yaml` / `occ-0`
...
```
      70:     - resolve(call_id, decision) resolves the pending decision
      71:     - set_notifier(notifier) installs a callback for approval state changes
      72:     """
      73:
      74:     def __init__(self, notifier: Callable[[], None] | None = None) -> None:
>>>   75:         self._futures: dict[
>>>   76:             str, asyncio.Future[ContinueDecision | DenyContinueDecision | AbortTurnDecision]
>>>   77:         ] = {}
>>>   78:         self._requests: dict[str, ApprovalRequest] = {}
      79:         self._lock = asyncio.Lock()
      80:         self._notifier = notifier
      81:
      82:     def set_notifier(self, notifier: Callable[[], None]) -> None:
      83:         """Install/replace the out-of-band notifier for approval state changes.
   ...
      89:     async def await_decision(
      90:         self, call_id: str, request: ApprovalRequest
      91:     ) -> ContinueDecision | DenyContinueDecision | AbortTurnDecision:
      92:         async with self._lock:
      93:             # Track the request so UIs can snapshot pending approvals
>>>   94:             self._requests[call_id] = request
      95:             fut = self._futures.get(call_id)
      96:             if fut is None:
      97:                 fut = asyncio.get_running_loop().create_future()
      98:                 self._futures[call_id] = fut
      99:         if self._notifier:
   ...
      93:             # Track the request so UIs can snapshot pending approvals
      94:             self._requests[call_id] = request
      95:             fut = self._futures.get(call_id)
...
>>>   98:                 self._futures[call_id] = fut
      99:         if self._notifier:
     100:             self._notifier()
     101:         return await fut
     102:
     103:     def resolve(self, call_id: str, decision: ContinueDecision | DenyContinueDecision | AbortTurnDecision) -> None:
   ...
      99:         if self._notifier:
     100:             self._notifier()
     101:         return await fut
     102:
     103:     def resolve(self, call_id: str, decision: ContinueDecision | DenyContinueDecision | AbortTurnDecision) -> None:
>>>  104:         fut = self._futures.pop(call_id, None)
>>>  105:         self._requests.pop(call_id, None)
     106:         if fut is not None and not fut.done():
     107:             fut.set_result(decision)
     108:         if self._notifier:
     109:             self._notifier()
     110:
   ...
     106:         if fut is not None and not fut.done():
     ...
     110:
>>>  111:     @property
>>>  112:     def pending(self) -> dict[str, ApprovalRequest]:
>>>  113:         """Public view of pending approval requests (immutable contract by convention)."""
>>>  114:         return self._requests
     115:
     116:
     117: # ---- Approval Policy Engine (decoupled, in-memory; optional) ----
     118:
     119:
```

### `policy-error-ambiguous-fields.yaml` / `occ-0`
...
### `redundant-on-response-override.yaml` / `occ-0`
...
```
     162:             await self.broadcast_status(True, active)
     163:
     164:     def set_session(self, session: AgentSession) -> None:
     165:         self._session = session
     166:
>>>  167:     def on_response(self, evt: Any) -> None:
>>>  168:         return None
     169:
     170:     def _spawn(self, coro: Awaitable[None]) -> None:
     171:         t: asyncio.Task[Any] = asyncio.create_task(coro)
     172:         self._bg_tasks.add(t)
     173:         t.add_done_callback(self._bg_tasks.discard)
   ...
      47: from adgn.mcp._shared.calltool import convert_fastmcp_result
      48:
      49: logger = logging.getLogger(__name__)
      50:
      51:
>>>   52: class ConnectionManager(BaseHandler):
      53:     def __init__(self) -> None:
      54:         self._clients: dict[int, tuple[WebSocket, asyncio.Queue[Any | None], asyncio.Task]] = {}
      55:         self._session: AgentSession | None = None
      56:         self._bg_tasks: set[asyncio.Task[Any]] = set()
      57:         self._event_id: int = 0
```
...
### `run-phase-string-literals.yaml` / `occ-0`
...
### `duplicate-mount-logic.yaml` / `occ-0`
...
```
      90:     # Mount registry server (provides global agent list and management)
      91:     registry_server = AgentRegistryBridgeServer(registry, global_comp)
      92:     await global_comp.mount_inproc("registry", registry_server)
      93:     logger.info("Mounted registry server")
      94:
>>>   95:     # Mount per-agent compositors for existing agents
>>>   96:     for agent_id in registry.known_agents():
>>>   97:         try:
>>>   98:             agent_comp = await create_agent_compositor(agent_id, registry)
>>>   99:             await global_comp.mount_inproc(f"agent{agent_id}", agent_comp)
>>>  100:             logger.info(f"Mounted agent compositor for agent {agent_id}")
>>>  101:         except Exception as e:
>>>  102:             logger.error(f"Failed to mount compositor for agent {agent_id}: {e}", exc_info=True)
>>>  103:             # Continue mounting other agents
     104:
     105:     # Standard infrastructure (resources aggregator, compositor metadata, admin)
     106:     if gateway_client is not None:
     107:         await mount_standard_inproc_servers(global_comp, gateway_client)
     108:         logger.info("Mounted standard infrastructure servers")
   ...
     108:         logger.info("Mounted standard infrastructure servers")
     109:
     110:     return global_comp
     111:
     112:
>>>  113: async def mount_agent_compositor_dynamically(
>>>  114:     global_compositor: Compositor,
>>>  115:     agent_id: AgentID,
>>>  116:     registry: InfrastructureRegistry
>>>  117: ) -> None:
>>>  118:     """Dynamically mount a new agent's compositor in the global compositor.
>>>  119:
>>>  120:     This is called when a new agent is created to add its resources to the
>>>  121:     global compositor without restarting.
>>>  122:
>>>  123:     Args:
>>>  124:         global_compositor: The global agents bridge compositor
>>>  125:         agent_id: ID of the newly created agent
>>>  126:         registry: Infrastructure registry
>>>  127:     """
>>>  128:     agent_comp = await create_agent_compositor(agent_id, registry)
>>>  129:     await global_compositor.mount_inproc(f"agent{agent_id}", agent_comp)
>>>  130:     logger.info(f"Dynamically mounted compositor for new agent {agent_id}")
     131:
     132:
     133: async def unmount_agent_compositor_dynamically(
     134:     global_compositor: Compositor,
     135:     agent_id: AgentID
```

### `duplicate-stub-creation.yaml` / `occ-0`
...
```
      42:
      43:
      44: async def test_validate_policy_valid(admin_server, docker_client: DockerClient):
      45:     """Test validating a valid policy."""
      46:     async with Client(admin_server) as session:
>>>   47:         stub = ApprovalPolicyAdminServerStub.from_server(admin_server, session)
>>>   48:         result = await stub.validate_policy(ValidatePolicyArgs(source="print('hello')"))
      49:
      50:     assert result.valid is True
      51:     assert_that(result.errors, has_length(0))
      52:
      53:
   ...
      52:
      53:
      54: async def test_validate_policy_syntax_error(admin_server):
      55:     """Test validating a policy with syntax errors."""
      56:     async with Client(admin_server) as session:
>>>   57:         stub = ApprovalPolicyAdminServerStub.from_server(admin_server, session)
>>>   58:         result = await stub.validate_policy(ValidatePolicyArgs(source="print('hello'"))
      59:
      60:     assert result.valid is False
      61:     assert_that(result.errors, has_item(contains_string("Syntax error")))
      62:
      63:
   ...
      62:
      63:
      64: async def test_validate_policy_runtime_error(admin_server, docker_client: DockerClient):
      65:     """Test validating a policy that fails at runtime."""
      66:     async with Client(admin_server) as session:
>>>   67:         stub = ApprovalPolicyAdminServerStub.from_server(admin_server, session)
>>>   68:         result = await stub.validate_policy(ValidatePolicyArgs(source="import sys; sys.exit(1)"))
      69:
      70:     assert result.valid is False
      71:     assert_that(result.errors, has_item(contains_string("Runtime validation failed")))
      72:
      73:
   ...
      79:     # Change engine's in-memory policy
      80:     engine.set_policy("print('different')")
      81:
      82:     # Reload from persistence
      83:     async with Client(admin_server) as session:
>>>   84:         stub = ApprovalPolicyAdminServerStub.from_server(admin_server, session)
>>>   85:         await stub.reload_policy(ReloadPolicyArgs(source=None))
      86:
      87:     # Engine should now have the persisted policy
      88:     current_policy, _ = engine.get_policy()
      89:     assert current_policy == "print('from persistence')"
      90:
   ...
      92: async def test_reload_policy_from_source(engine, admin_server, docker_client: DockerClient):
      93:     """Test reloading policy from provided source."""
      94:     # Reload with provided source
      95:     new_source = load_default_policy_source()
      96:     async with Client(admin_server) as session:
>>>   97:         stub = ApprovalPolicyAdminServerStub.from_server(admin_server, session)
>>>   98:         await stub.reload_policy(ReloadPolicyArgs(source=new_source))
      99:
     100:     # Engine should have the new source
     101:     current_policy, _ = engine.get_policy()
     102:     assert current_policy == new_source
     103:
   ...
     104:
     105: async def test_reload_policy_validates_source(admin_server, docker_client: DockerClient):
     106:     """Test that reload validates the source before setting."""
     107:     # Try to reload with invalid source
     108:     async with Client(admin_server) as session:
>>>  109:         stub = ApprovalPolicyAdminServerStub.from_server(admin_server, session)
>>>  110:         with pytest.raises(Exception):  # Should fail validation
     111:             await stub.reload_policy(ReloadPolicyArgs(source="import sys; sys.exit(1)"))
     112:
     113:
     114: async def test_reload_policy_no_persistence_raises(engine, persistence):
     115:     """Test that reloading from empty persistence raises error."""
   ...
     126:
     127:     new_admin_server = ApprovalPolicyAdminServer(engine=new_engine)
     128:
     129:     # Try to reload (should fail - no policy in persistence)
     130:     async with Client(new_admin_server) as session:
>>>  131:         stub = ApprovalPolicyAdminServerStub.from_server(new_admin_server, session)
>>>  132:         with pytest.raises(ValueError, match="No policy found in persistence"):
     133:             await stub.reload_policy(ReloadPolicyArgs(source=None))
```
...
### `optional-compositor-param.yaml` / `occ-0`

File: `adgn/src/adgn/agent/mcp_bridge/servers/registry_bridge.py`
...
```
      48:     Resources (after mounting):
      49:     - resource://registry/agents/list - List all agents with status
      50:     - resource://registry/agents/{id}/info - Specific agent information
      51:     """
      52:
>>>   53:     def __init__(self, registry: InfrastructureRegistry, global_compositor: Compositor | None = None):
      54:         super().__init__(name="registry")
      55:         self._registry = registry
      56:         self._global_compositor = global_compositor
      57:         self._register_resources()
...
>>>   56:         self._global_compositor = global_compositor
      ...
      58:         self._register_tools()
      59:
      60:     def _register_resources(self) -> None:
      61:         @self.resource("resource://agents/list", name="agents_list", mime_type="application/json")
   ...
     147:             """
     148:             # Create agent infrastructure
     149:             await self._registry.create_agent(agent_id)
     150:
     151:             # Dynamically mount the new agent's compositor if we have a global compositor
>>>  152:             if self._global_compositor is not None:
>>>  153:                 from adgn.agent.mcp_bridge.compositor_factory import mount_agent_compositor_dynamically
>>>  154:
>>>  155:                 await mount_agent_compositor_dynamically(
>>>  156:                     global_compositor=self._global_compositor,
>>>  157:                     agent_id=agent_id,
>>>  158:                     registry=self._registry
>>>  159:                 )
     160:
     161:             # Notify that agents list changed
     162:             await self.notify_agents_list_changed()
     163:
     164:             return {"agent_id": agent_id, "status": "created"}
   ...
     172:
     173:             Returns:
     174:                 Dictionary with agent_id and status
     175:             """
     176:             # Dynamically unmount the agent's compositor if we have a global compositor
>>>  177:             if self._global_compositor is not None:
>>>  178:                 from adgn.agent.mcp_bridge.compositor_factory import unmount_agent_compositor_dynamically
>>>  179:
>>>  180:                 await unmount_agent_compositor_dynamically(
>>>  181:                     global_compositor=self._global_compositor,
>>>  182:                     agent_id=agent_id
>>>  183:                 )
     184:
     185:             # Remove agent infrastructure
     186:             await self._registry.remove_agent(agent_id)
     187:
     188:             # Notify that agents list changed
```
...
```
     256:
     257:         local_result_map: dict[str, CallToolResult] = {
...
>>>  265:         ) -> ToolCallOutcome:
     ...
     269:                 if (cached := copy.deepcopy(local_map[cid])).is_error:
     270:                     return ToolCallFailure(result=cached, reason=_maybe_error_message(cached))
   ...
     271:                 return ToolCallSuccess(result=cached)
     272:
...
>>>  280:                 args = val
     ...
     283:             if res.is_error:
     284:                 return ToolCallFailure(result=res, reason=_maybe_error_message(res))
     285:             return ToolCallSuccess(result=res)
   ...
     300:             cancelled_exc = anyio.get_cancelled_exc_class()
     301:
     302:             async def runner(fc: FunctionCallItem) -> None:
...
>>>  305:                     outcome = await invoker(fc, fc.arguments)
   ...
   ...
     329:
     330:     async def _run_tool_calls_sequential(
...
>>>  334:             outcome = await invoker(function_call, function_call.arguments)
     ...
     338:                     self._emit_tool_result(remaining, _abort_result())
     339:                 self.finished = True
```

### `redundant-count-fields.yaml` / `occ-0`

File: `adgn/src/adgn/agent/mcp_bridge/servers/approvals_bridge.py`
...
```
      33:
      34: class ApprovalsResponse(BaseModel):
      35:     """Response containing all approvals for an agent (pending + decided history)."""
      36:     agent_id: AgentID
      37:     approvals: list[ApprovalItem]
>>>   38:     pending_count: int
      39:     decided_count: int
      40:
      41:
      42: class ApprovalsBridgeServer(NotifyingFastMCP):
      43:     """MCP server exposing approval resources for agents bridge.
   ...
      60:     def _register_resources(self) -> None:
      61:         @self.resource("resource://approvals", name="approvals", mime_type="application/json")
      62:         async def get_approvals() -> ApprovalsResponse:
      63:             """Get all approvals for this agent (pending + decided history)."""
      64:             approvals_list = []
>>>   65:             pending_count = 0
      66:             decided_count = 0
      67:
      68:             # Add pending approvals
      69:             pending_map = self._hub.pending
      70:             for call_id, tool_call in pending_map.items():
   ...
      75:                         status=ApprovalStatus.PENDING,
      76:                         reason=None,
      77:                         timestamp=datetime.now(),  # Approx timestamp for pending
      78:                     )
      79:                 )
>>>   80:                 pending_count += 1
      81:
      82:             # Add decided approvals from persistence
      83:             records = await self._persistence.get_tool_call_records(self._agent_id)
      84:             for record in records:
      85:                 if record.decision is not None:
   ...
     111:             approvals_list.sort(key=lambda x: x.timestamp, reverse=True)
     112:
     113:             return ApprovalsResponse(
     114:                 agent_id=self._agent_id,
     115:                 approvals=approvals_list,
>>>  116:                 pending_count=pending_count,
     117:                 decided_count=decided_count,
     118:             )
     119:
     120:     def _register_tools(self) -> None:
     121:         @self.tool()
```

### `test-functions-bundle-cases.yaml` / `occ-0`

File: `adgn/tests/agent/persist/test_integration.py`
...
```
     350:             assert retrieved.execution is None
     351:
     352:
     353: # Test 5: Complex CallToolResult test
     354: @pytest.mark.asyncio
>>>  355: async def test_complex_calltoolresult_content_types(persistence: SQLitePersistence, test_agent: str) -> None:
>>>  356:     """Test persistence of complex CallToolResult with multiple content types.
>>>  357:
>>>  358:     Tests:
>>>  359:     - Text content
>>>  360:     - Image content
>>>  361:     - Error content
>>>  362:     - Mixed content (multiple items)
>>>  363:     - Empty content
>>>  364:     """
>>>  365:     # Test 1: Text content
>>>  366:     record1 = ToolCallRecord(
>>>  367:         call_id="content-text",
>>>  368:         run_id=None,
>>>  369:         agent_id=test_agent,
>>>  370:         tool_call=ToolCall(name="text_tool", call_id="content-text", args_json="{}"),
>>>  371:         decision=Decision(outcome=ApprovalOutcome.POLICY_ALLOW, decided_at=datetime.now(UTC), reason=None),
>>>  372:         execution=ToolCallExecution(
>>>  373:             completed_at=datetime.now(UTC),
>>>  374:             output=mcp_types.CallToolResult(
>>>  375:                 content=[mcp_types.TextContent(type="text", text="Simple text output")], isError=False
>>>  376:             ),
>>>  377:         ),
>>>  378:     )
>>>  379:     await persistence.save_tool_call(record1)
>>>  380:     retrieved1 = await persistence.get_tool_call("content-text")
>>>  381:     assert retrieved1 is not None
>>>  382:     assert_that(retrieved1.execution.output.content, has_length(1))
>>>  383:     assert_that(retrieved1.execution.output.content[0], instance_of(mcp_types.TextContent))
>>>  384:     assert retrieved1.execution.output.content[0].text == "Simple text output"
>>>  385:
>>>  386:     # Test 2: Image content
>>>  387:     record2 = ToolCallRecord(
>>>  388:         call_id="content-image",
>>>  389:         run_id=None,
>>>  390:         agent_id=test_agent,
>>>  391:         tool_call=ToolCall(name="image_tool", call_id="content-image", args_json="{}"),
>>>  392:         decision=Decision(outcome=ApprovalOutcome.POLICY_ALLOW, decided_at=datetime.now(UTC), reason=None),
>>>  393:         execution=ToolCallExecution(
>>>  394:             completed_at=datetime.now(UTC),
>>>  395:             output=mcp_types.CallToolResult(
>>>  396:                 content=[
>>>  397:                     mcp_types.ImageContent(
>>>  398:                         type="image",
>>>  399:                         data="iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==",
...
>>>  405:         ),
>>>  406:     )
>>>  407:     await persistence.save_tool_call(record2)
>>>  408:     retrieved2 = await persistence.get_tool_call("content-image")
>>>  409:     assert retrieved2 is not None
>>>  410:     assert_that(retrieved2.execution.output.content, has_length(1))
>>>  411:     assert_that(retrieved2.execution.output.content[0], instance_of(mcp_types.ImageContent))
>>>  412:     assert retrieved2.execution.output.content[0].mimeType == "image/png"
>>>  413:
>>>  414:     # Test 3: Error content
>>>  415:     record3 = ToolCallRecord(
>>>  416:         call_id="content-error",
>>>  417:         run_id=None,
>>>  418:         agent_id=test_agent,
>>>  419:         tool_call=ToolCall(name="error_tool", call_id="content-error", args_json="{}"),
>>>  420:         decision=Decision(outcome=ApprovalOutcome.POLICY_ALLOW, decided_at=datetime.now(UTC), reason=None),
>>>  421:         execution=ToolCallExecution(
>>>  422:             completed_at=datetime.now(UTC),
>>>  423:             output=mcp_types.CallToolResult(
>>>  424:                 content=[mcp_types.TextContent(type="text", text="Error: File not found")], isError=True
>>>  425:             ),
>>>  426:         ),
>>>  427:     )
>>>  428:     await persistence.save_tool_call(record3)
>>>  429:     retrieved3 = await persistence.get_tool_call("content-error")
>>>  430:     assert retrieved3 is not None
>>>  431:     assert retrieved3.execution.output.isError is True
>>>  432:     assert retrieved3.execution.output.content[0].text == "Error: File not found"
>>>  433:
>>>  434:     # Test 4: Mixed content (text + image)
>>>  435:     record4 = ToolCallRecord(
>>>  436:         call_id="content-mixed",
>>>  437:         run_id=None,
>>>  438:         agent_id=test_agent,
>>>  439:         tool_call=ToolCall(name="mixed_tool", call_id="content-mixed", args_json="{}"),
>>>  440:         decision=Decision(outcome=ApprovalOutcome.POLICY_ALLOW, decided_at=datetime.now(UTC), reason=None),
>>>  441:         execution=ToolCallExecution(
>>>  442:             completed_at=datetime.now(UTC),
>>>  443:             output=mcp_types.CallToolResult(
>>>  444:                 content=[
>>>  445:                     mcp_types.TextContent(type="text", text="Generated visualization:"),
>>>  446:                     mcp_types.ImageContent(type="image", data="base64data", mimeType="image/svg+xml"),
>>>  447:                     mcp_types.TextContent(type="text", text="Analysis complete."),
>>>  448:                 ],
>>>  449:                 isError=False,
>>>  450:             ),
>>>  451:         ),
>>>  452:     )
>>>  453:     await persistence.save_tool_call(record4)
>>>  454:     retrieved4 = await persistence.get_tool_call("content-mixed")
>>>  455:     assert retrieved4 is not None
>>>  456:     assert_that(retrieved4.execution.output.content, has_length(3))
>>>  457:     assert_that(retrieved4.execution.output.content[0], instance_of(mcp_types.TextContent))
>>>  458:     assert_that(retrieved4.execution.output.content[1], instance_of(mcp_types.ImageContent))
>>>  459:     assert_that(retrieved4.execution.output.content[2], instance_of(mcp_types.TextContent))
>>>  460:
>>>  461:     # Test 5: Empty content (edge case)
>>>  462:     record5 = ToolCallRecord(
>>>  463:         call_id="content-empty",
>>>  464:         run_id=None,
>>>  465:         agent_id=test_agent,
>>>  466:         tool_call=ToolCall(name="empty_tool", call_id="content-empty", args_json="{}"),
>>>  467:         decision=Decision(outcome=ApprovalOutcome.POLICY_ALLOW, decided_at=datetime.now(UTC), reason=None),
>>>  468:         execution=ToolCallExecution(
>>>  469:             completed_at=datetime.now(UTC), output=mcp_types.CallToolResult(content=[], isError=False)
>>>  470:         ),
>>>  471:     )
>>>  472:     await persistence.save_tool_call(record5)
>>>  473:     retrieved5 = await persistence.get_tool_call("content-empty")
>>>  474:     assert retrieved5 is not None
>>>  475:     assert_that(retrieved5.execution.output.content, has_length(0))
>>>  476:
     477:
     478: # Test 6: Concurrent access test
     479: @pytest.mark.asyncio
     480: async def test_concurrent_access_and_data_integrity(persistence: SQLitePersistence, test_agent: str) -> None:
     481:     """Test concurrent access patterns to ensure no data corruption.
   ...
     528:         assert retrieved.execution.output.content[0].text == f"Result {i}"
     529:
     530:
     531: # Test 7: Error handling test
     532: @pytest.mark.asyncio
>>>  533: async def test_error_handling_and_data_validation(tmp_path: Path, test_agent: str) -> None:
>>>  534:     """Test error handling for invalid data scenarios.
>>>  535:
>>>  536:     Tests:
>>>  537:     - Invalid JSON in database (manual corruption)
>>>  538:     - Missing required fields
>>>  539:     - Malformed data
>>>  540:     """
>>>  541:     db_path = tmp_path / "error_test.db"
>>>  542:     persist = SQLitePersistence(db_path)
>>>  543:     await persist.ensure_schema()
>>>  544:
>>>  545:     # Create a test agent using ORM
>>>  546:     from adgn.agent.persist.models import Agent
>>>  547:
>>>  548:     async with persist._session() as session:
>>>  549:         agent = Agent(
>>>  550:             id=test_agent,
>>>  551:             created_at=datetime.now(UTC),
>>>  552:             mcp_config={},
>>>  553:             preset="test",
>>>  554:         )
>>>  555:         session.add(agent)
>>>  556:         await session.commit()
>>>  557:
>>>  558:     # First, save a valid record
>>>  559:     valid_record = ToolCallRecord(
>>>  560:         call_id="valid-call",
>>>  561:         run_id=None,
>>>  562:         agent_id=test_agent,
>>>  563:         tool_call=ToolCall(name="valid_tool", call_id="valid-call", args_json="{}"),
>>>  564:         decision=None,
>>>  565:         execution=None,
>>>  566:     )
>>>  567:     await persist.save_tool_call(valid_record)
>>>  568:
>>>  569:     # Test 1: Manually corrupt the JSON in the database
>>>  570:     from sqlalchemy import text
>>>  571:
>>>  572:     async with persist._session() as session:
>>>  573:         await session.execute(
>>>  574:             text("UPDATE tool_calls SET tool_call_json = :json WHERE call_id = :call_id"),
>>>  575:             {"json": '{"invalid json syntax', "call_id": "valid-call"},
>>>  576:         )
>>>  577:         await session.commit()
>>>  578:
>>>  579:     # Attempting to retrieve should raise an error
>>>  580:     with pytest.raises(Exception):  # Could be JSONDecodeError or Pydantic validation error
>>>  581:         await persist.get_tool_call("valid-call")
>>>  582:
>>>  583:     # Test 2: Insert record with missing required field in JSON
>>>  584:     async with persist._session() as session:
>>>  585:         # Insert a record missing the 'name' field in tool_call
>>>  586:         await session.execute(
>>>  587:             text("""
>>>  588:             INSERT INTO tool_calls (
>>>  589:                 call_id, run_id, agent_id, tool_call_json,
>>>  590:                 decision_json, execution_json,
>>>  591:                 created_at, decided_at, completed_at
>>>  592:             ) VALUES (:call_id, :run_id, :agent_id, :tool_call_json, :decision_json, :execution_json, :created_at, :decided_at, :completed_at)
>>>  593:             """),
>>>  594:             {
>>>  595:                 "call_id": "missing-field",
>>>  596:                 "run_id": None,
>>>  597:                 "agent_id": test_agent,
>>>  598:                 "tool_call_json": '{"call_id": "missing-field"}',  # Missing 'name' field
>>>  599:                 "decision_json": None,
>>>  600:                 "execution_json": None,
>>>  601:                 "created_at": datetime.now(UTC).isoformat(),
>>>  602:                 "decided_at": None,
>>>  603:                 "completed_at": None,
>>>  604:             },
>>>  605:         )
>>>  606:         await session.commit()
>>>  607:
>>>  608:     # Attempting to retrieve should raise a validation error
>>>  609:     with pytest.raises(Exception):  # Pydantic ValidationError
>>>  610:         await persist.get_tool_call("missing-field")
>>>  611:
>>>  612:     # Test 3: Get non-existent call_id (should return None, not raise)
>>>  613:     result = await persist.get_tool_call("non-existent-id")
>>>  614:     assert result is None
>>>  615:
>>>  616:     # Test 4: Test with malformed timestamp
>>>  617:     async with persist._session() as session:
>>>  618:         await session.execute(
>>>  619:             text("""
>>>  620:             INSERT INTO tool_calls (
>>>  621:                 call_id, run_id, agent_id, tool_call_json,
>>>  622:                 decision_json, execution_json,
>>>  623:                 created_at, decided_at, completed_at
>>>  624:             ) VALUES (:call_id, :run_id, :agent_id, :tool_call_json, :decision_json, :execution_json, :created_at, :decided_at, :completed_at)
>>>  625:             """),
>>>  626:             {
>>>  627:                 "call_id": "bad-timestamp",
>>>  628:                 "run_id": None,
>>>  629:                 "agent_id": test_agent,
>>>  630:                 "tool_call_json": '{"name": "tool", "call_id": "bad-timestamp"}',
>>>  631:                 "decision_json": '{"outcome": "policy_allow", "decided_at": "not-a-timestamp", "reason": null}',
>>>  632:                 "execution_json": None,
>>>  633:                 "created_at": datetime.now(UTC).isoformat(),
>>>  634:                 "decided_at": "not-a-timestamp",  # Invalid timestamp
>>>  635:                 "completed_at": None,
>>>  636:             },
>>>  637:         )
>>>  638:         await session.commit()
>>>  639:
>>>  640:     # Attempting to retrieve should raise an error
     641:     with pytest.raises(Exception):  # Could be ValueError or Pydantic validation error
     642:         await persist.get_tool_call("bad-timestamp")
     643:
     644:
     645: # Summary test: Verify all lifecycle states exist in a single run
   ...
     283:     # That's application logic. The persistence layer just stores what it's given.
     284:
     285:
     286: # Test 4: Decision outcome variants test
     287: @pytest.mark.asyncio
>>>  288: async def test_decision_outcome_variants(persistence: SQLitePersistence, test_agent: str) -> None:
>>>  289:     """Test all decision outcome types and their execution patterns.
>>>  290:
>>>  291:     Tests:
>>>  292:     - POLICY_ALLOW → successful execution
>>>  293:     - POLICY_DENY_ABORT → no execution
>>>  294:     - USER_APPROVE → successful execution
>>>  295:     - USER_DENY_ABORT → no execution
>>>  296:     - USER_DENY_CONTINUE → no execution but could continue
>>>  297:     - POLICY_DENY_CONTINUE → no execution but could continue
>>>  298:     """
>>>  299:     outcomes_to_test = [
>>>  300:         (ApprovalOutcome.POLICY_ALLOW, True, "Policy auto-approved and executed"),
>>>  301:         (ApprovalOutcome.POLICY_DENY_ABORT, False, "Policy denied, aborted"),
>>>  302:         (ApprovalOutcome.USER_APPROVE, True, "User approved, executed"),
>>>  303:         (ApprovalOutcome.USER_DENY_ABORT, False, "User denied, aborted"),
>>>  304:         (ApprovalOutcome.USER_DENY_CONTINUE, False, "User denied but continued"),
>>>  305:         (ApprovalOutcome.POLICY_DENY_CONTINUE, False, "Policy denied but continued"),
>>>  306:     ]
>>>  307:
>>>  308:     for i, (outcome, should_execute, reason) in enumerate(outcomes_to_test):
>>>  309:         call_id = f"outcome-{i}"
>>>  310:         decision = Decision(outcome=outcome, decided_at=datetime.now(UTC), reason=reason)
>>>  311:
>>>  312:         if should_execute:
>>>  313:             # Create COMPLETED record with execution
>>>  314:             record = ToolCallRecord(
>>>  315:                 call_id=call_id,
>>>  316:                 run_id=None,
>>>  317:                 agent_id=test_agent,
>>>  318:                 tool_call=ToolCall(name=f"tool_{i}", call_id=call_id, args_json="{}"),
>>>  319:                 decision=decision,
>>>  320:                 execution=ToolCallExecution(
>>>  321:                     completed_at=datetime.now(UTC),
>>>  322:                     output=mcp_types.CallToolResult(
>>>  323:                         content=[mcp_types.TextContent(type="text", text=f"Executed for {outcome}")], isError=False
>>>  324:                     ),
>>>  325:                 ),
>>>  326:             )
>>>  327:         else:
>>>  328:             # Create EXECUTING record without execution (denied)
>>>  329:             record = ToolCallRecord(
>>>  330:                 call_id=call_id,
>>>  331:                 run_id=None,
>>>  332:                 agent_id=test_agent,
>>>  333:                 tool_call=ToolCall(name=f"tool_{i}", call_id=call_id, args_json="{}"),
>>>  334:                 decision=decision,
>>>  335:                 execution=None,
>>>  336:             )
>>>  337:
>>>  338:         await persistence.save_tool_call(record)
>>>  339:
>>>  340:         # Verify
>>>  341:         retrieved = await persistence.get_tool_call(call_id)
>>>  342:         assert retrieved is not None
>>>  343:         assert retrieved.decision is not None
>>>  344:         assert retrieved.decision.outcome == outcome
>>>  345:         assert retrieved.decision.reason == reason
>>>  346:
>>>  347:         if should_execute:
>>>  348:             assert retrieved.execution is not None
>>>  349:         else:
>>>  350:             assert retrieved.execution is None
>>>  351:
     352:
...
### `verbose-enum-mapping.yaml` / `occ-0`
...
```
      ...
      84:             for record in records:
      85:                 if record.decision is not None:
>>>   86:                     # Map ApprovalOutcome to ApprovalStatus
>>>   87:                     if record.decision.outcome == ApprovalOutcome.APPROVED:
>>>   88:                         status = ApprovalStatus.APPROVED
>>>   89:                     elif record.decision.outcome == ApprovalOutcome.REJECTED:
>>>   90:                         status = ApprovalStatus.REJECTED
>>>   91:                     elif record.decision.outcome == ApprovalOutcome.DENIED:
>>>   92:                         status = ApprovalStatus.DENIED
>>>   93:                     elif record.decision.outcome == ApprovalOutcome.ABORTED:
>>>   94:                         status = ApprovalStatus.ABORTED
>>>   95:                     else:
>>>   96:                         # Fallback for unknown outcomes
>>>   97:                         status = ApprovalStatus.REJECTED
      98:
      99:                     approvals_list.append(
     100:                         ApprovalItem(
     101:                             call_id=record.tool_call.id,
     102:                             tool_call=record.tool_call,
```
