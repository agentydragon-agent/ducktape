...
from props.display import SHORT_SHA_LENGTH, ColumnDef, build_status_columns, build_table_from_schema, fmt_pct, short_sha
...
_OCCURRENCE_COLUMNS: list[ColumnDef[Any, Any]] = [
    ...
    ColumnDef(
        "Catchable", lambda r: r.n_catchable_occurrences, lambda v: fmt_float(v, decimals=1), justify="right", width=9
    ),
    *_STATUS_COLUMNS,
]
...
def cmd_stats(ctx: typer.Context) -> None:
    ...
    def format_view_stats(stats: RecallByDefinitionSplitKind | None, fully_computed: bool = False) -> tuple[str, ...]:
        """Format view stats for the multi-column-group table as (recall, lcb, n, zero, stuck, context).
