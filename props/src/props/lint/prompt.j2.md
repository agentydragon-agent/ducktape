...
- Workspace: mounted read-only at {{ compositor.container_working_dir }} (analysis only)
- Scratch/caches/logs: write under /tmp (do not modify files under the workspace)

## Input Schemas:

{% for name in ["IssueCore", "Occurrence", "LineRange", "LintSubmitPayload", "IssueLintFindingRecord"] %}
- {{ name }}
```json
