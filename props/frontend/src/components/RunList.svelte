...
<div class="bg-white dark:bg-gray-900 rounded-lg shadow dark:shadow-gray-950/30 p-4">
  ...
  {#if activeRuns.length === 0}
    <p class="text-gray-500 dark:text-gray-400 text-sm">No active runs</p>
  {:else}
    <div class="space-y-2">
      {#each activeRuns as run (run.agent_run_id)}
        <div
          class="block w-full text-left p-3 rounded border dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
        >
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
              <span class="text-xs text-gray-500 dark:text-gray-400">{run.model}</span>
