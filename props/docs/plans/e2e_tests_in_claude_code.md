...
1. **`.claude/skills/test_props/start_infra_podman.sh`** - Partial implementation that starts:
...
2. **When triggered, the session hook will**:
   ...
   - Run `.claude/skills/test_props/start_infra_podman.sh` to start containers only (no bazel)
