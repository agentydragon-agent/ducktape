...
- Booleans: clear state/predicate names. Past-participle adjectives are fine (enabled, accepted, archived, verified) and read as state; use is*/has* when a bare noun would be ambiguous (is_admin, has_license). Avoid bare nouns like license/admin/feature.
