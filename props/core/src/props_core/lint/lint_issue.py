...
from agent_core.handler import AbortIf, BaseHandler, SequenceHandler
from agent_core.loop_control import InjectItems, RequireAnyTool
...
from openai_utils.model import FunctionCallItem, OpenAIModelProto, UserMessage
from props_core.db.models import Snapshot
from props_core.db.session import get_session
from props_core.docker_env import PROPERTIES_DOCKER_IMAGE, PropertiesDockerCompositor
from props_core.ids import BaseIssueID, SnapshotSlug
from props_core.lint.rich_renderers import render_to_rich
from props_core.models.lint import IssueLintFindingRecord, LintSubmitPayload, extract_corrections
from props_core.models.true_positive import IssueCore, LineRange, Occurrence
...
class LintIssueCompositor(PropertiesDockerCompositor):
    ...
    def __init__(self, workspace_root: Path, docker_client: aiodocker.Docker, submit_state: LintSubmitState):
        super().__init__(
            workspace_root,
            docker_client,
            image_id=PROPERTIES_DOCKER_IMAGE,
            db_conn=None,
            workspace_mode="ro",
            network_mode="none",
            extra_env=None,
...
def make_linter_bootstrap_calls(
    ...
    if dirs:
        calls.append(
            docker_exec_call(
                builder, compositor.runtime, cmd=["ls", "-la", *[compositor.container_working_dir / d for d in dirs]]
            )
        )
    ...
    if files and not big_detected:
        for q in files:
            if sizes[str(q)] > BIG_THRESHOLD:
                continue
            paths_to_number.append(compositor.container_working_dir / q)
...
def make_bootstrap_calls_for_inspection(
    ...
    return [
        ...
        docker_exec_call(builder, compositor.runtime, cmd=["ls", "-la", compositor.container_working_dir]),
    ]
...
async def lint_issue_run(
    ...
    *,
    ...
    Requires content_root to be provided - caller is responsible for fetching
    snapshot content from the database and extracting to a temporary directory.
...
async def run_specimen_lint_issue_async(
    ...
    *,
    ...
    with get_session() as session:
        ...
        # Validated occurrence exists - tp_occ would be used in reimplementation
        _ = tp_orm.occurrences[occurrence_index]

    # lint-issue command is temporarily disabled
    # TODO: Reimplement to fetch snapshot content from database using extract_snapshot_to_temp()
    raise NotImplementedError(
        "lint-issue command is temporarily disabled. "
        "Hydration was removed - snapshots are now stored in PostgreSQL. "
        "Reimplement using extract_snapshot_to_temp() from props_core.agent_helpers or delete this command."
    )
