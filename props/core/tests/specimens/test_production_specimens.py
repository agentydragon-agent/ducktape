...
NOTE: Most schema/format validations are enforced by the database during sync:
- Split enum values (DB column constraint)
- Issue ID format (CHECK constraint)
- File references exist (FK to snapshot_files)
- Line numbers valid (validate_line_numbers_trigger)
- Pydantic models (validated during YAML parsing)
...
def module_monkeypatch() -> Generator[pytest.MonkeyPatch, None, None]:
...
async def synced_production_db(module_monkeypatch: pytest.MonkeyPatch) -> AsyncGenerator[DatabaseConfig, None]:
