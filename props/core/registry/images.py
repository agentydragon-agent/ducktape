...
- OCI image refs (e.g., "localhost:5050/critic:v1" or "sha256:...")
...
# Registry URL for pulling images (internal docker network)
REGISTRY_URL = os.environ.get("PROPS_REGISTRY_CONTAINER_NAME", "props-registry")
REGISTRY_PORT = os.environ.get("PROPS_REGISTRY_CONTAINER_PORT", "5000")
...
async def resolve_image_id(
    ...
    Args:
        ...
        image_ref: OCI image reference (e.g., "localhost:5050/critic:v1")
    ...
    Raises:
        ValueError: If neither image_ref nor definition_id is provided
    """
...
def _normalize_image_ref(image_ref: str) -> str:
    ...
    - "sha256:abc..." -> unchanged (digest reference)
    - "critic:v1" -> "props-registry:5000/critic:v1" (add registry)
    - "localhost:5050/critic:v1" -> unchanged (already has registry)
    """
    # Digest references are already complete
    if image_ref.startswith("sha256:"):
        return image_ref

    # If no registry (no / before first :), add default registry
    if "/" not in image_ref.split(":")[0]:
        return f"{REGISTRY_URL}:{REGISTRY_PORT}/{image_ref}"
